...
class fx_connectControl extends mobileHomeControl {
    ...
    public function get_stateOp() {
        $logic_connect_api = Logic('fx_connect_api');
        ...
        if(!empty($_GET['code'])) {
            ...
            if(!empty($user_info['unionid'])){
                ...
                if($token) {
                    ...
                    redirect(WAP_SITE_URL.'/tmpl/fx_member/member.html?info=hao&username='.$state_data['username'].'&key='.$state_data['key']);
                } else {
                    output_error('会员登录失败');
                }
            } else {
