...
class member_refundControl extends mobileMemberControl {
    ...
    public function refund_all_formOp(){
        ...
        $condition['order_state'] = array('in',array(ORDER_STATE_NEW,ORDER_STATE_TO_BC,
            ORDER_STATE_MAKING,
            ORDER_STATE_TO_SIGN,
            ORDER_STATE_PAY,
            ORDER_STATE_TOSEND,
        ...
        if(!empty($order_info) && is_array($order_info)) {
            ...
            $order['rcb_amount'] = ncPriceFormat($order_info['rcb_amount']);
            ...
            //$order['allow_refund_amount'] = $order['rcb_amount'];
           $order['book_amount'] = ncPriceFormat($book_amount);
            ...
            $allow_refund_amount = bcsub($order['rcb_amount'], $order_info['refund_amount']);
            foreach($order_goods_list as $key => $value) {
                ...
                $goods['goods_img_360'] = $value['goods_image'];
                ...
                //$allow_refund_amount = bcadd($allow_refund_amount, $value['goods_pay_price']);
            }
            $order['allow_refund_amount'] = $allow_refund_amount;
    ...
    public function refund_all_postOp(){
        ...
        $return_form = intval($_POST['return_form']);
        $model_trade = Model('trade');
        $order_paid = $model_trade->getOrderState('order_paid');//订单状态20:已付款
        ...
        if(isset($refund['seller_state']) && $refund['seller_state']==1){
            output_error('已有待审核未处理的退款单');
        }
        if (empty($order_info) || $payment_code == 'offline' || $refund['refund_id'] > 0) {//检查数据,防止页面刷新不及时造成数据错误
            ...
            $allow_refund_amount = bcsub($order_info['rcb_amount'], $order_info['refund_amount'],2);
            $refund_array = array();
            ...
            $refund_array['return_form'] = $return_form;
            ...
            if ($state) {
                //$updata = array();
                //$updata['refund_amount'] = bcadd($allow_refund_amount, $order_info['refund_amount']);
                //if($updata['refund_amount'] == $order_info['order_amount']){
                    //$refund_state = 2;
                //}else{
                    //$refund_state = 1;
                //}
                ...
                //$condition = array();
                //$condition['order_id'] = $order_id;
                //$model_order->editOrder($updata, $condition);

                $model_refund->editOrderLock($order_id);
                $data = array();
                $data['order_id'] = $order_id;
                $data['log_role'] = "seller";
                $data['log_msg'] = "申请全部退款,流水单号：".$state;
    ...
    public function refund_formOp(){
        ...
        if($_GET['refund_type']==2){
            $checkGoods = $model_order->checkGoodsReturn($goods_id);
            if($checkGoods['state']==false){
                output_error($checkGoods['msg']);
            }
        }
        $condition = array();
        $condition['store_id'] = $this->member_info['store_id'];
        $return_check = $this->check_seller_limit('limit_return_apply');
        if(!$return_check){
            $condition['buyer_id'] = $member_id;
        }
        $condition['order_id'] = $order_id;
        ...
        if($refund_state == 1 && $goods_id > 0) {
            ...
            $goods['is_zp'] = $goods_info['goods_type'] == 5?true:false;
            //$goods['goods_img_360'] = thumb($goods_info,360);
            $goods['goods_img_360'] = $goods_info['goods_image'];
            ...
            $goods['goods_pay_price'] = $goods_info['goods_pay_price'];

            $rcb_amount = $order_info['rcb_amount'];//已支付金额
            $goods_refund_amount = $goods_info['refund_amount'];//订单退款金额
            $goods_pay_price = $goods_info['goods_pay_price'];//商品实际成交价
            //计算出可退款金额
            $orderInfo = $model_refund->getRightOrderList($condition);
            $goods_amount = $return_price = $breach_amount = 0;
            if(isset($orderInfo['goods_list'])){
                foreach ($orderInfo['goods_list'] as $k => $val) {
                    if($val['is_return'] == 0){
                        $goods_amount = bcadd($goods_amount, ncPriceFormat($val['goods_pay_price']), 2);
                    }
                    //$return_price = bcadd($return_price, ncPriceFormat($val['refund_amount']), 2);
                    //$breach_amount = bcadd($breach_amount, ncPriceFormat($val['breach_amount']), 2);
                }
            }
            $return_price = $orderInfo['refund_amount'];
            $breach_amount = $orderInfo['breach_amount'];
            $scale = $order['order_amount']>0?bcdiv($goods_pay_price, $goods_amount ,10):0;
            $rcb_amount = bcsub($rcb_amount, $return_price, 2);//减去退款金额
            $rcb_amount = bcsub($rcb_amount, $breach_amount, 2);//减去退款违约金
            $can_return_price = round(bcmul($rcb_amount, $scale, 2));//退款金额
           // $can_return_price = $breach_amount;

            $goods['goods_return_price'] = ncPriceFormat($can_return_price);//退货可退金额
           // $rest_amount = $rcb_amount - $order_refund_amount; //剩余已支付金额
            $rest_amount = $rcb_amount;
            $goods_rest_amount = bcsub($goods_pay_price, $goods_refund_amount, 2); //成交金额减去已退金额
            $goods_return_price = $goods_rest_amount > $rest_amount ? $rest_amount : $goods_rest_amount;
            $goods['goods_refund_price'] = ncPriceFormat($goods_return_price);
            // $goods['goods_refund_price'] = ncPriceFormat($rest_amount);//退款可退金额
            /*
            if($order_info['pay_status'] != 3){
                $goods_pay_price = $order_info['rcb_amount'];//未付全款则最多不能超过退定金金额
            }else{
                $goods_pay_price = $goods_info['goods_pay_price'];//商品实际成交价
    ...
    public function refund_postOp(){
        ...
        $refund_type = intval($_POST['refund_type']);
        $return_type = intval($_POST['return_type']);
        $return_form = intval($_POST['return_form']);
        $model_order = Model('order');
        $model_refund = Model('refund_return');
        ...
        $condition['seller_state'] = array('lt','2');
        $refund = $model_refund->getRefundReturnInfo($condition);
        if(!empty($refund)){
            output_error('记录已提交');
        }
        if($refund_state == 1 && $goods_id > 0 && empty($refund)) {
            ...
            if ($order_info['order_state'] >= ORDER_STATE_SEND) {                
                if(!empty($goods_info['goods_itemid'])){
                    $itemsinfo = $model_goodsitems->getGoodsItemInfo(array('goods_id'=>$goods_info['goods_itemid']), 'weixiu_status');
                    if(!in_array($itemsinfo['weixiu_status'], array(0,1,4))){
                        output_error('申请退货时货品'.$goods_info['goods_itemid'].'维修状态只能是未申请，维修取消或者维修完成!');  
                    }
                }else{
                    if($refund_type == 2) {
                        output_error('退货货号不能为空，且在库存列表查询得到');
                    }
                }
            }
            ...
            if($return_type == 2){//退商品才填写违约金
                $breach_amount = floatval($_POST['breach_amount']);//违约金额
            }else{
                $breach_amount = 0;//违约金额
            }
            ...
            //    $goods_pay_price = $order_amount - $order_refund_amount;
            //}
            $rest_amount = bcsub($order_info['rcb_amount'], $order_refund_amount, 2);
            $goods_return_price = $goods_pay_price > $rest_amount ? $rest_amount : $goods_pay_price;
            if($goods_return_price != 0){
            ...
            $refund_array['return_type'] = $return_type;//退货类型:1为不用退货,2为需要退货
            if (!in_array($refund_array['refund_type'], [1,2])) {
                output_error('退货类型错误');
            }

            $refund_array['seller_state'] = '1';//状态:1为待审核,2为同意,3为不同意
            $refund_array['refund_amount'] = ncPriceFormat($refund_amount);
            $refund_array['breach_amount'] = ncPriceFormat($breach_amount);
            ...
            if ($state) {
                ...
                $refund_type = $refund_type == 2 && $order_info['order_state'] >= ORDER_STATE_SEND ? "退货" : "退款";
                //添加订单日志
                $data = array();
                $data['order_id'] = $order_id;
                $data['log_role'] = "seller";
                $data['log_msg'] = "申请".$refund_type.",".($return_type == 1 ? '不退商品' : '退商品');
                $data['log_user'] = $this->member_info['member_name'];
                $data['log_orderstate'] = $order_info['order_state'];
                $model_order->addOrderLog($data);
                output_data(1);
