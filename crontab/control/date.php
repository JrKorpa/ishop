...
class dateControl extends BaseCronControl {
    ...
    public function indexOp() {
        ...
        $this->_del_member_goods_expired();
        //更新订单商品佣金值
        ...
        for ($stime = $start_time; $stime < time(); $stime = $stime+86400){
            ...
            for ($i=0; $i<$countnum; $i+=100){//每次查询100条
                ...
                if ($orderlog_list){
                    ...
                    //查询订单数据
                    //$field = 'order_id,order_sn,store_id,buyer_id,buyer_name,buyer_phone,add_time,audit_time,payment_code,order_amount,shipping_fee,evaluation_state,order_state,refund_state,refund_amount,rcb_amount,breach_amount,order_from,pay_status,is_zp,payment_time,customer_source_id,seller_id,seller_name,goods_amount';
                    $field = '*';
                    $order_list_tmp = $model->table('orders')->field($field)->where(array('audit_time'=>array('neq',''),'order_state'=>array('gt',ORDER_STATE_TO_CONFIRM),'pay_status'=>array('gt',ORDER_PAY_TODO),'order_id'=>array('in',$orderid_arr)))->select();
                    $order_list = array();

                    $orderid_arr = array();//重新获取订单ID
                    foreach ((array)$order_list_tmp as $k=>$v){
                        //判读订单是否计入统计（在线支付订单已支付或者经过退款的取消订单或者货到付款订单订单已成功）
                        $v['order_isvalid'] = 0;
                        /*
                        if ($v['payment_code'] != 'offline' && $v['pay_status'] > ORDER_PAY_TODO && $v['order_state'] != ORDER_STATE_CANCEL){//在线支付并且已支付并且未取消
                            $v['order_isvalid'] = 1;
                        ...
                        */
                        if($v['pay_status'] > ORDER_PAY_TODO){
                             $v['order_isvalid'] = 1;
                        }
                    ...
                    //查询订单商品
                    #$field = 'rec_id,order_id,goods_itemid,goods_name,goods_price,goods_num,goods_image,goods_pay_price,store_id,buyer_id,goods_type,promotions_id,commis_rate,gc_id,from_type,is_xianhuo,style_sn';
                    $field = '*';
                    ...
                    foreach ((array)$ordergoods_list as $k=>$v){
                        $goodsid_arr[] = $v['goods_itemid'];
                    }
    
                    //查询商品信息
                    $field = 'goods_id,yuanshichengbenjia as goods_price,cat_type';
                    $goods_list_tmp = $model->table('goods_items')->field($field)->where(array('goods_id'=>array('in',$goodsid_arr)))->select();
                    /*
                    ...
                    */
                    ...
                    foreach ((array)$ordergoods_list as $k=>$v){
                        ...
                        $tmp['order_add_time'] = strtotime($order_list[$v['order_id']]['audit_time']);
                        ...
                        $tmp['cat_type'] = $goods_list[$v['goods_itemid']]['cat_type'];
                        $tmp['from_type'] = $v['from_type'];
                        $tmp['is_xianhuo'] = $v['is_xianhuo'];
                        $tmp['style_sn'] = $v['style_sn'];
                        $tmp['tuo_type'] = $v['tuo_type'];
                        $tmp['cert_type'] = $v['cert_type'];
                        $tmp['carat'] = $v['carat'];
                        $tmp['caizhi'] = $v['caizhi'];
                        $tmp['jinzhong'] = $v['jinzhong'];
                        $tmp['bc_status'] = $v['bc_status'];
                        $tmp['cert_id'] = $v['cert_id'];
                        $tmp['breach_amount'] = $v['breach_amount'];
                    ...
                    foreach ((array)$order_list as $k=>$v){
                        ...
                        $tmp['order_add_time'] = strtotime($v['audit_time']);
                        ...
                        $tmp['buyer_phone'] = $v['buyer_phone'];
						$tmp['is_zp'] = $v['is_zp'];
						$tmp['customer_source_id'] = $v['customer_source_id'];
						$tmp['seller_id'] = $v['seller_id'];
						$tmp['seller_name'] = $v['seller_name'];
                        $tmp['rcb_amount'] = $v['rcb_amount'];
                        $tmp['breach_amount'] = $v['breach_amount'];
                        $tmp['pay_status'] = $v['pay_status'];
                        $tmp['payment_time'] = $v['payment_time'];
                        $tmp['goods_amount'] = $v['goods_amount'];//商品总价格
                        $tmp['delay_time'] = $v['delay_time'];//目前发货时间
                        $tmp['finnshed_time'] = $v['finnshed_time'];//完成时间
                        $tmp['remark'] = $v['remark'];//完成时间
                        $order_insert_arr[] = $tmp;
                    }
                    $res = $model->table('stat_order')->insertAll($order_insert_arr);
                    /*
                    if(!$res){
                        $sql = $model->table('stat_order')->insertAll11($order_insert_arr);
                        $this->loginfo('sql',$sql);
                    }
                    /*
                    if(!$res){
                        foreach ($order_insert_arr as $val){
                            $model->table('stat_order')->insert($val);
                        }
                    }
                    */
    ...
    /**
     * 删除过期member_goods 缓存信息
     */
    private function _del_member_goods_expired(){
        $date = date("Y-m-d 00:00:00",strtotime("-2 days"));       
        Model("member_goods")->where(array('update_time'=>array('lt',$date)))->delete();
    }
