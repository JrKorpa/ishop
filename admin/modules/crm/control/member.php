...
class memberControl extends SystemControl{
    ...
    private $links = array(
        ...
        array('url'=>'act=member&op=discount_set','type'=>'discount_set','lang'=>'member_discount_set'),
    );


    public function discount_setOp(){
        //输出子菜单
        Tpl::output('top_link',$this->sublink($this->links,'discount_set'));
        Tpl::setDirquna('crm');
        Tpl::showpage('member.discount_set');
    }
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if (!empty($_POST['member_passwd'])){
                    $update_array['member_passwd'] = xmd5($_POST['member_passwd']);
                }
    ...
    public function get_xmlOp() {
        ...
        foreach ($member_list as $value) {
          /*  $param = array();
            ...
            $param = array();
            $operation="<a class='btn blue' href='index.php?act=member&op=member_edit&member_id=" . $value['member_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            $operation .= "<span class='btn'><em><i class='fa fa-cog'></i>设置 <i class='arrow'></i></em><ul>";
            $operation .= "<li><a href='index.php?act=member&op=predeposit_add&member_id=". $value['member_id'] ."&member_name=".urlencode($value["member_name"])."'>押金调节</a></li>";
            $operation .= "<li><a href='index.php?act=member&op=discount_set&member_id=". $value['member_id'] ."&member_name=".urlencode($value["member_name"])."'>折扣设置</a></li>";
            $operation .= "</ul></span>";
