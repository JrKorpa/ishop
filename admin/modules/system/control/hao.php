...
class haoControl extends SystemControl{
    ...
    public function active_jikeOp() {
        include BASE_DATA_PATH . '/api/smsapi/JikeCrm.php';
        $jike = new jikeCrm();
        $result = $jike->register();
        showMessage('操作完成：' . $result);
    }
			/**
	 * 默认微信公众号设置 
	 */
	public function webchatOp(){
		...
		if (chksubmit()){
            ...
            $update_array['hao_webchat_AesK'] = $_POST['hao_webchat_AesK'];
			$result = $model_setting->updateSetting($update_array);
			if ($result === true){
				$this->log(L('nc_edit,sms_set'),1);
				showMessage(L('nc_common_save_succ'));
