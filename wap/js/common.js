...
if(tel){
    sessionStorageAdd("tel", tel, true);
}
...
/*
$("#login_common").click(function(){
    var key = getCookie('key');
    if (key) {
        if(window.confirm('您确认要退出登录吗？')){
            delCookie('username');
            delCookie('key');
            delCookie('store_list');
        ...
        areaSelected: function(options) {
            ...
            function _init() {
                ...
                var html = '<div id="areaSelected">'
                    ...
                    + '<div class="header header-msk">'
                    + '<div class="header-wrap">'
                    + '<div class="header-l h_left"><a href="javascript:void(0);"><i class="icon icon-arwLeft"></i></a></div>'
                    + '<div class="header-title h_title">'
                    + '选择地区'
                    + '</div>'
                    + '<div class="header-r h_right"><a href="javascript:void(0);"><i class="icon cion-close"></i></a></div>'
                    + '</div>'
                    + '</div>'
            ...
            function _getAreaList() {
                $.ajax({//获取区域列表
                    type:'get',
                    url:ApiUrl+'/index.php?act=management&ent=address&op=area_list',
                    data:{area_id:ASID},
                    dataType:'json',
                    async:false,
                    success:function(result){
                        ...
                        for(var i=0;i<data.area_list.length;i++){
                            area_li += '<li><a href="javascript:void(0);" data-id="' + data.area_list[i].area_id + '" data-name="' + data.area_list[i].area_name + '"><h4>' + data.area_list[i].area_name + '</h4><span class="arrow-r icon icon-arwRight"></span> </a></li>';
                        }
...
window.platformId = (function(){
    var platform = getCookie('cliplatf');
    if (!platform) {
        platform = getQueryString('platf');
    }

    if (platform) {
        addCookie('cliplatf', platform, 3600 * 24);
    }
...
function get_network_state() {
    ...
    try{
        ...
        if (!state) state = navigator.onLine;
    }

    return state;
}

function notify_tips(tips) {
    $.sDialog({
        skin: "green",
        content: tips,
        okBtn: false,
...
(function(){
    ...
    $.ajax = function(options){
        ...
        if (typeof options.cache_key === 'function') options.cache_key = options.cache_key();
        if (options.cache_key) resolve_cache(options, state); 

        _ajax_(options);
    };

    var _alert_ = window.alert;
    window.alert = function(message) {
       /* if (typeof window.cordovaext !== 'undefined') {
            window.cordovaext.notification().alert(message, null, '提示', '知道了');
        } else */
        if (typeof $.sDialog == 'function') {
            notify_tips(message);
        } else {
            _alert_(message);
        }
...
function resolve_cache(options, network_state) {
    /**
     *  默认为速度优先，cache_way=1：网络优先， cache_way=2：缓存优先
     */
    var cache = localStorage.getItem(options.cache_key);
    var cacheway = options.cache_way;
    var _ori_success = options.success;
    var keep_ori_success_call = true;

    if (cache) {
        if (typeof LZString !== 'undefined') cache = LZString.decompress(cache);
        
        cache = JSON.parse(cache);
        if (cacheway === undefined || network_state === false || network_state === 'none') {
            //console.log('0');
            if (cache.d !== undefined) cache = cache.d;
            _ori_success(cache);
            keep_ori_success_call = false;
    
            // we got the data, so that we ignore the error handler.
            ...
            //console.log('2');            
            // 缓存优先，仅在缓存数据无效的时候，发起网络请求
            var now = new Date().getTime() / 1000;
            if (cache.__ed && cache.__ed > now) {
                if (cache.d !== undefined) cache = cache.d;
                _ori_success(cache);      
                keep_ori_success_call = false;

                // 缓存有效，暂不需要发起API调用
                options.beforeSend = function(xhr, settings) { return false; }
    ...
    options.success = function(result) {
        if (keep_ori_success_call) _ori_success(result);

        if (cacheway == '2') {
            var now = new Date();
            var lifespan = options.cache_mi ? options.cache_mi : 30;
            ...
            result.__ed = now.getTime() / 1000;
        }

        var resultStr = JSON.stringify(result);
        if (typeof LZString !== 'undefined') resultStr = LZString.compress(resultStr);
        localStorage.setItem(options.cache_key, resultStr);
        //console.log('store and build....');
    }
}
...
if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
    if (window.platformId) {
        // for mobile
        loadJs(WapSiteUrl +'/js/native/fastclick.js');
...
function logout() {
    ...
    if (loginCfg.sso) {
        delCookie(loginCfg.token_ck);
        window.location.href = loginCfg.logout;
    } else {
...
if (loginCfg.sso) {
    ...
    if (!token) {
        ...
        if (token) {
            $.ajax({
                type: 'post',
                url: ApiUrl + "/index.php?act=login&op=check_token",
                data: {token: token},
                dataType: 'json',
                success: function (result) {
                    //console.log(result);
                    if (result.datas.error) {
                        logout();
                    } else if (result.datas != true) {
                        addCookie(loginCfg.token_ck, result.datas, 24);
                    }
                }
            });
        }else{
            if (/platf=(ios|android|osx)$/.test(window.location.href) == false) {
                window.location.href = loginCfg.login;
            }
        }
    }
