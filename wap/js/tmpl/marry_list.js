...
$(function(){
    ...
    template.helper('console', function(o){
        console.log(o);
    });
    ...
    $(document).delegate("#proclass-list a",'click',function(){
        ...
        var cat_type = $(this).attr('data-val');
        initPage(cat_type);
        //doSearch();
    });
...
function tabSearch (cat_type) {
    var key = getCookie('key');
    $.ajax({
        url:ApiUrl+"/index.php?act=marry_goods&op=marry_goods_tab",
        type:'post',
        data:{key:key},
        dataType:'json',
        success:function(result){
            if(result.code ==200){
                var params = _history?sessionStorage.getItem('marry_list_params'):null;
                params = JSON.parse(params);
                result.datas.active_type = "";
                if(params){
                    result.datas.active_type = params.cat_type;
                    if(params.cat_type == "" || params.cat_type == undefined){
                        result.datas.active_type=2;
                    }
                }else{
                    result.datas.active_type=2;
                }
                //控制从首页过来的数据选中吊坠3,默认选中钻戒2
                var from = getQueryString('from');
                if(from=="home")  result.datas.active_type=3;
                var cat_type = result.datas.active_type;
...
function initPage (cat_type) {
    //if(!cat_type) cat_type=2;
    var xilie = getQueryString('xilie');
    var key = getCookie('key');
    $.ajax({
        url:ApiUrl+"/index.php?act=marry_goods&op=marry_goods_index",
        type:'post',
...
function getSearchVal (e) {
    ...
    data['cat_type'] = cat_type == ''?2:cat_type;
...
function cutTabs (e) {
 ...
 //重置页面
    function doReset () {
        sessionStorageAdd("marry_list_params", [], true);
...
function doSearch (clickPage,params) {
    ...
    if (typeof params == 'undefined' || params.length ==0) {
        ...
        params['key'] = key;
        params['gc_parent_id'] = 1057;
        params['search_type'] = cuttab;
        if(cuttab=="diff"){
            params['contrast_list'] =  sessionStorageGet("marryContrastList"); 	
        }
    }
    ...
    sessionStorage.setItem('marry_list_params', JSON.stringify(params));
    ...
    $.ajax({
        ...
        success:function(result){
            if(result.code ==200){
                ...
                if(sessionStorageGet("marryContrastList")) result.datas.diffNum=sessionStorageGet("marryContrastList").length;
                ...
                pagenav(page, result.page_count);
            }else{
                alert(result.datas.error);
            }
        }
...
function conTrast (e) {
    ...
    var diffinfo = sessionStorageGet("marryContrastList");
...
function delConTrast(goods_sn){
	...
	marryContrastList.splice(existsIndex,1);
