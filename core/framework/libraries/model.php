...
class Model{
    ...
    public function __call($method,$args) {
        ...
        }else if(strtolower($method)=='groupcount'){
            //group分页
            $field =  isset($args[0])?$args[0]:'*';
            $options['field'] = $field;
            $options =  $this->parse_options($options);
            $sql = $this->db->buildSelectSql($options);
            $sql = "select count(*) from ({$sql}) t limit 1";
...
class ModelDb{
    ...
    protected $comparison      = array('null'=>'is null','notnull'=>'is not null','eq'=>'=','neq'=>'<>','gt'=>'>','egt'=>'>=','lt'=>'<','elt'=>'<=','notlike'=>'NOT LIKE','like'=>'LIKE','in'=>'IN','not in'=>'NOT IN');
    // 查询表达式
    ...
    protected function parseWhereItem($key,$val) {
        ...
        if(is_array($val)) {
            if(is_string($val[0])) {
                ...
                }else if (preg_match('/^(NULL|NOTNULL)$/i',$val[0])){
                    $whereStr .= $key.' '.$this->comparison[strtolower($val[0])];
                }else if (preg_match('/^(HASLEN)$/i',$val[0])){
                    $whereStr .= 'LENGTH('.$key.')>0';
                }else if (preg_match('/^(NOLEN)$/i',$val[0])){
                    $whereStr .= 'LENGTH('.$key.')=0';
...
//                    $whereStr .= ' ('.$key.' '.$val[1].') ';
                ...
                $count = count($val);
                if(!is_array($val[$count-1])&&in_array(strtoupper(trim($val[$count-1])),array('AND','OR','XOR'))) {
