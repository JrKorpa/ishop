    ...
    public function show($style = null){
        ...
        switch ($this->style) {
            case '1':
                ...
                break;
            case '2':
                $html_totalnum_a = (1+$this->getEachNum()*($this->getNowPage()-1));
                $html_totalnum_b = $this->getEachNum()*$this->getNowPage();
                $html_totalnum_b = $html_totalnum_b >$this->getTotalNum()?$this->getTotalNum():$html_totalnum_b;
                $html_totalnum = $html_totalnum_a.'-'.$html_totalnum_b.'/'.$this->getTotalNum();
                $html_totalpage = $this->getNowPage().'/'.$this->getTotalPage();
                $html_page .= '<ul>';
                $html_page .="<li><span>记录：".$html_totalnum."</span></li>";
                $html_page .="<li><span>页码：".$html_totalpage."</span></li>";
                if ($this->getNowPage() <= 1){
