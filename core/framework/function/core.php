...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('INTELLIGENT_SYS') or exit('Access Invalid!'); return ".$data.";";
 ...
 * @return string
 */
function urlAgent($act = '', $op = '', $args = array()){
    return url($act, $op, $args, false, AGENT_SITE_URL);
}
/**
 * 验证是否为平台店铺
 *
...
/***
 * 获取数据仓库实体
 * @param unknown $repository_define
 * @throws Exception
 * @return unknown
 */
function data_gateway($repository_define) {
    $repositories = $GLOBALS['config']["repository"];
    if ($repositories && array_key_exists($repository_define, $repositories)) {
        $config = $repositories[$repository_define];
        
        @include_once(BASE_DATA_PATH.DS.'repository'.DS.'interface'.DS.$repository_define.".php");     
        @include_once(BASE_DATA_PATH.DS.'repository'.DS."base_repository.php");    
        @include_once(BASE_DATA_PATH.DS.'repository'.DS.$config['class'].".php");
        
        if (!isset($config['url'])) $config['url'] = $repositories['base_url'];
        
        if (is_subclass_of($config['class'], $repository_define)) {
            return new $config['class']($config);
        }
        
        throw new Exception($config['class']. ' is not instance of '. $repository_define);        
    }

    throw new Exception('can not find the implement of repository: '. $repository_define);
}
 ...
 */
function xmd5($password, $suffix = "") {
    return md5(md5($password . 'kela~/!@#$%^&') . $suffix);
}

function formatWapPageList($data,$list_key="list",$show_role = false){
    $result = array(
        "code"=>200,
        "hasmore"=>false,
        "page_total"=>0,
    ...
    if($show_role == true){
        ...
        $result['datas']['role'] = $role;
    }
    if(!empty($data["data"])){
        $result['hasmore'] = $data['isLast']==1?false:true;
        $result['page'] = $data['page'];
        $result['page_size'] = $data['pageSize'];
        $result['record_count'] = $data['recordCount'];
        $result['page_count'] = $data['pageCount'];
        $result['page_total'] = $data['pageCount'];
        $result['datas'][$list_key] = $data['data'];
    }
    return $result;
}


function exportCSV($xls_content,$filename){
    ini_set('memory_limit', -1);
    set_time_limit(0);
    ob_end_clean();
    header("Content-type: text/html; charset=gbk");
    header("Content-type:aplication/vnd.ms-excel");
    header("Content-Disposition:filename=" . iconv("utf-8", "gbk", $filename . date("Y-m-d")) . ".csv");
    echo iconv("utf-8", "GB18030", $xls_content);

    exit;
}

function tovalInArray($array1, $array2)
{
    $return_array = array();
    if(!empty($array1)){
        //$return_array = array_intersect($array1, $array2);
