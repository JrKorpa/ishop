...
function getStatData_Column2D($stat_arr){
    ...
    foreach ($stat_arr['series'] as $series_k=>$series_v){
        $series_v['color'] = $color[$series_k];
        $stat_arr['series'][$series_k] = $series_v;
    }
...
function getStatData_Pie($data){
	...
	$stat_arr['tooltip']['pointFormat'] = '{series.name}: <b>{point.percentage:.1f} % </b>({point.y})';
	$stat_arr['plotOptions']['pie'] = array(
        ...
        'showInLegend'=>true,//图例是否显示
	);
	//图例内容
    $legend = isset($data['legend']) ? $data['legend']:array(
 ...
 * 获得饼形图数据（特殊用于损益报表）
 */
function getStatData_Pie1($data){
    $stat_arr['chart']['type'] = 'pie';
    $stat_arr['credits']['enabled'] = false;
    $stat_arr['title']['text'] = $data['title'];
