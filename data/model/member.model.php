...
class memberModel extends Model {
	...
	*/
    ...
    public function editMember($condition, $data) {
        ...
        if ($update && $condition['member_id']) {
            ...
            if (isset($data['member_passwd'])) {
                EventEmitter::dispatch('ishop', ['event' => 'member_upsert', 'member_id' => $condition['member_id'], 'member_passwd' => $data['member_passwd']]);
            }
        }
        return $update;
    }
    ...
    //根据用户名查询用户信息
    public function get_member_by_name($user_name){
        $condition['member_name'] = $user_name;
        $member_info = $this->getMemberInfo($condition);
        if (!empty($member_info)) {
            if(!$member_info['member_state']){
     ...
     * @return  array 数组格式的返回结果
     */
    public function addMember($param,$transMode = true) {
        if(empty($param)) {
            return false;
        }
        try {
            ...
            $member_info['member_passwd']       = xmd5(trim($param['member_passwd']));
