...
class refund_returnModel extends Model{
    ...
    public function addRefundReturn($refund_array, $order = array(), $goods = array()) {
        ...
        if (!empty($order) && is_array($order)) {
            ...
            $refund_array['store_name'] = !empty($order['store_name'])?$order['store_name']:'test';
     ...
     * @return int
     */
    public function addHaReturn($refund_array, $goods = array()) {

        if (!empty($goods) && is_array($goods)) {
            $refund_array['goods_id'] = $goods['goods_id'];
            $refund_array['order_goods_id'] = '0';
            $refund_array['order_goods_type'] = '0';
            $refund_array['goods_name'] = $goods['goods_name'];
            $refund_array['commis_rate'] = '0';
            $refund_array['goods_image'] = '0';
        }
        $refund_array['refund_sn'] = $this->getRefundsn($refund_array['store_id']);
        $refund_id = $this->table('refund_return')->insert($refund_array);
        return $refund_id;
    ...
    public function getRefundState($order) {
        ...
        if($pay_status> ORDER_PAY_TODO || $order_state> ORDER_STATE_SEND){
            $refund = '1';
        }else{
            $refund = '0';
        }
    ...
    /**
     * 根据订单明细ID查询要商品列表
     * @param unknown $order_detail_id
     */
    public function getReturnGoodsByDetailId($order_detail_id){
        $sql = "select distinct g.*,bg.sale_price from erp_bill_goods bg inner join erp_bill b on bg.bill_id=b.bill_id inner join goods_items g on bg.goods_itemid=g.goods_id where bg.order_detail_id={$order_detail_id} and b.bill_status=2";
