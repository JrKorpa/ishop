...
class voucherModel extends Model {
    ...
    /**
     * 检查券码是否有效
     * @param $voucher_code 券码
     */
    public function checkAvailableVoucher($voucher_code)
    {
        $condition['voucher_end_date'] = array('gt', TIMESTAMP);
        $condition['voucher_state'] = 1;
        $condition['voucher_code'] =$voucher_code;
        $voucher_info = $this->table('voucher')->where($condition)->find();
        if(!empty($voucher_info)) return true;
        return false;
    }
     ...
     * 获得代金券详情
     */
    public function getVoucherInfo($where = array(), $field = '*', $order = 'voucher_id desc',$group = '') {
     ...
     * goods_info 商品信息
     */
    public function get_diamond_type($user_id, $goods_info){
        ...
        $diainfo = array();
        $type = 0;
        $cert = isset($goods_info['cert']) ? $goods_info['cert']:'';
        $cert_id = isset($goods_info['cert_id']) ? $goods_info['cert_id']:'';
        $style_sn = isset($goods_info['style_sn']) ? $goods_info['style_sn']:'';
        $product_type = isset($goods_info['product_type']) ? $goods_info['product_type']:'';
        $cat_type = isset($goods_info['cat_type']) ? $goods_info['cat_type']:'';
        $carat = isset($goods_info['carat'])? $goods_info['carat']:'';
        if($style_sn == 'DIA' || ($product_type=="钻石" && $cat_type=="裸石")){
            //裸钻根据证书类型判断；
            if($cert_id){
                $diainfo = $diamond_api->GetDiamondByCert_id($cert_id);
            }
            if($diainfo['error'] != 1){
                $cert = isset($diainfo['return_msg']['cert'])?$diainfo['return_msg']['cert']:'';
                $carat  = isset($diainfo['return_msg']['carat'])?$diainfo['return_msg']['carat']:'';
            }
            if($cert == 'HRD-S'){
                if($carat<0.5){
                    $type = 5;
            ...
            //普通空拖、成品
            if(!in_array($type,array(10,12,13))){
                $type = 11;
            }
        }
        return $type;
