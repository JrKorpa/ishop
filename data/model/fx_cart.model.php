...
class fx_cartModel extends Model {
    ...
    private function _addCartDb($goods_info = array(),$quantity) {
        //验证购物车商品是否已经存在
        $condition = array();
        $condition['fx_goods_id'] = $goods_info['goods_id'];
        ...
        $array['agent_id']  = $goods_info['agent_id'];
        $array['fx_goods_id']  = $goods_info['goods_id'];
        ...
        $array['agent_name'] = $goods_info['agent_name'];
        $array['bl_id'] = isset($goods_info['bl_id']) ? $goods_info['bl_id'] : 0;
        return $this->table('fx_cart')->insert($array);
//         return $this->insert($array);
    ...
    private function _addCartCookie($goods_info = array(), $quantity = null) {
        //去除斜杠
        $cart_str = get_magic_quotes_gpc() ? stripslashes(cookie('fx_cart')) : cookie('fx_cart');
        ...
        $cart_array[$goods_info['goods_id']] = array(
          'agent_id' => $goods_info['agent_id'],
          'fx_goods_id' => $goods_info['goods_id'],
          'goods_name' => $goods_info['goods_name'],
          'goods_price' => $goods_info['goods_price'],
          'goods_image' => $goods_info['goods_image'],
          'goods_num' => $quantity
        );
        setNcCookie('fx_cart',encrypt(base64_encode(serialize($cart_array))),24*3600);
        return true;
    }
     ...
     * @return array
     */
    public function listCart($type, $condition = array(), $limit = null) {
        if ($type == 'db') {
    ...
    public function delCart($type, $condition = array()) {
        if ($type == 'db') {
            ...
            $cart_str = base64_decode(decrypt($cart_str));
            $cart_array = @unserialize($cart_str);
            if (key_exists($condition['fx_goods_id'],(array)$cart_array)) {
                unset($cart_array[$condition['fx_goods_id']]);
            }
            setNcCookie('fx_cart',encrypt(base64_encode(serialize($cart_array))),24*3600);
            $result = true;
        }
     ...
     *
     */
    public function mergecart($member_info = array(), $agent_id = null){
        ...
        //批量添加购物车
        $this->batchAddCart($cart_new_list, $member_info['member_id']);
        //最后清空登录前购物车内容
     ...
     */
    public function batchAddCart($cart_new_list, $member_id, $agent_id=0){
...
//         }
//         $where['fx_goods_id'] = array('in',array_keys($cart_new_list));
        $goods_list = array();
        foreach ($cart_new_list as $k => $cart_goods){
            $condition['fx_goods_id'] = $k;
            $goods = $model_goods->infoGoods($condition);
            $goods['goods_num'] = $cart_goods['goods_num'];
            $goods_list[] = $goods;
        }
        
        file_put_contents(time().'Test000', json_encode($cart_new_list));
        file_put_contents(time().'Test', json_encode($goods_list));
        if (!empty($goods_list)){
            foreach ($goods_list as $k=>$goods_info){
                file_put_contents(time().'Test0_'.$k, json_encode($goods_info));
                $goods_info['goods_id'] = $goods_info['fx_goods_id'];
                $goods_info['buyer_id'] = $member_id;
                file_put_contents(time().'Test1_'.$k, json_encode($goods_info));
                $this->addCart($goods_info,'db',$goods_info['goods_num']);
            }
        }
    }
}
