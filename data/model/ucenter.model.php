...
class ucenterModel{
	...
	public function userLogin($username, $password) {
		if($GLOBALS['setting_config']['ucenter_type'] == 'phpwind') {
			$user_login = uc_user_login($username,xmd5($password),0);
		...
		if($uid > 0) {
			...
			if(empty($user_exit)) {
				...
				$user_array['member_passwd']		= xmd5($pwd);
				...
				if(xmd5($pwd) != $user_exit['member_passwd']) {
					Db::update('member',array('member_passwd'=>xmd5($pwd)),"where member_id='$uid' and member_name='$uname'");
				}
	...
	public function outputLogin($uid, $password){
		if($GLOBALS['setting_config']['ucenter_type'] == 'phpwind') {
		    $user_login = uc_user_login($uid,xmd5($password),1);
