...
class goodsModel extends Model{
    ...
    public function editGoodsById($update, $goodsid_array, $updateXS = false) {
        ...
        $result = $this->table('goods_items')->where($condition)->update($update);
        if ($result) {
     ...
     */
    public function getGoodsMarketList($condition, $pageObj){
        $condition_str = $this->_condition($condition);
        $condition_str .= " and goods.goods_state != ". self::STATE10;
        $param = array(
            'table'=>'goods,goods_common',
            'field'=>'goods.*,goods_common.goods_costprice',
            'where'=>$condition_str,
            'join_type'=>'left join',
            'join_on'=>array('goods.goods_commonid=goods_common.goods_commonid')
        );
    ...
    public function _condition($condition){
        ...
        if ($condition['goods_id'] != ''){
            $condition_str .= " and goods.goods_id = ". $condition['goods_id'] ."";
        }
        if ($condition['agent_id'] != ''){
            $condition_str .= " and goods.agent_id = ". $condition['agent_id'] ."";
        }
        if ($condition['store_id'] != ''){
            $condition_str .= " and goods.store_id = ". $condition['store_id'] ."";
        }
        if ($condition['agent_name'] != ''){
            $condition_str .= " and agent_name = '". $condition['agent_name'] ."'";
        }
        if ($condition['goods_name'] != ''){
            $condition_str .= " and goods.goods_name LIKE '%". $condition['goods_name'] ."%'";
        }
