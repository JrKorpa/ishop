...
class buy_1Logic {
    ...
    /**
     * API 获取商品信息(商品返回字段可以增加调整)
     * @param unknown $goods_id
     * @param unknown $goods_type
     * add by gaopeng 2018-03-31
     */
    public function apiGetGoodsInfo($goods_id,$goods_type,$store_id,$company_id,$member_id=0,$load_member_goods=true,$is_shop=1){
        if($goods_type == 1){
            //空托
            ...
            $where = array(
                'goods_sn'=>$goods_id,
                'channel_id'=>$store_id,
                'extends' =>array('goods_image','goods_price','goods_shape')
            );
            $result = $styleApi->get_style_goods_info($where);
            //print_r($result);exit;
            if(isset($result['error']) && $result['error']==0){
                $data = $result['return_msg'];
            }else{
                return callback(false,"商品不存在");
            }   
            $goods_price = round($data['goods_price']);
            if(empty($data['goods_image'])) {
                $data['goods_image'] = BASE_SITE_URL."/data/common/default_goods_image_240.gif";
            }
            $goods_info = array(
                'goods_id'=>$data['goods_sn'],
                'style_sn'=>$data['style_sn'],
                'goods_name'=>$data['style_name'],
                ...
                'caizhi'=>$data['caizhi'],
                'caizhi_name'=>$data['caizhi_name'],
                'yanse'=>$data['yanse'],
                'yanse_name'=>$data['yanse_name'],
                'xiangkou'=>$data['xiangkou'],
                'shoucun'=>$data['shoucun'],
                "jinzhong"=>$data['weight'],
                "goods_type"=>$goods_type,
                "is_xianhuo"=>0,
                "goods_status"=>$data['is_ok'],
                "goods_num"=>1,
                "goods_storage"=>999999,//库存
                "is_book"=>0,
            );
        }else if($goods_type == 2){
            //裸钻
            $diamondApi = data_gateway('idiamond');
            $result = $diamondApi->get_diamond_info(array('cert_id'=>$goods_id));
            if(isset($result['error']) && $result['error']==0){
                $data = $result['return_msg'];
            }else{
                return callback(false,"商品不存在");
            }
            $diamondApi->multiply_jiajialv($data,$store_id, $company_id);
            $goods_price = round($data['shop_price']);
            //$goods_image = !empty($data['img'])?$data['img']:'/wap/images/img/01.jpg';
            $goods_image = BASE_SITE_URL.'/wap/images/img/01.jpg';
            //if(substr($goods_image,0,63)=='http://diamonds.kirangems.com/GemKOnline/DiaSearch/appVideo.jsp'
            //    || substr($goods_image,0,48)=='https://diamanti.s3.amazonaws.com/images/diamond'){
            //    $goods_image = "/wap/images/img/01.jpg";
            //}
            $goods_info = array(
                "goods_id"=>$data['goods_sn'],
                "style_sn"=>"DIA",
                ...
                "goods_type"=>$goods_type,
                "is_xianhuo"=>$data['good_type']==1?1:0,
                "goods_status"=>$data['status'],
                "goods_num"=>1,
                "goods_storage"=>1,//库存
                "is_book"=>0,
                "kuan_sn"=>$data['kuan_sn'],
                'jiajialv'=>$data['jiajialv'],
                'pifajia'=>$data['pifajia'],
            );
        }else if($goods_type == 3){
            //起版
            $purchaseApi = data_gateway("ipurchase");
            $result = $purchaseApi->get_qiban_info($goods_id);
            if(isset($result['error']) && $result['error']==0){
                $data = $result['return_msg'];
            }else{
                return callback(false,$result['error_msg']);
            }           
            $goods_price = round($data['price']);
            if(empty($data['goods_image'])) {
                $data['goods_image'] = BASE_SITE_URL."/data/common/default_goods_image_240.gif";
            }

            //如果添加起版选择的是 1：工厂配钻，工厂镶嵌，则是成品，或者是空托
            if($data['xuqiu'] == 1){
                $tuo_type = 1;
                $is_cpdz = 1;
            }else{
            ...
            $goods_info = array(
                "goods_id"=>$data['addtime'],
                "style_sn"=>$data['kuanhao'],
                "goods_name"=>$data['goods_name'],
                "goods_price"=>$goods_price,
                "goods_pay_price"=>$goods_price,
                'goods_hand_price'=>$goods_price,
                "goods_image"=>$data['goods_image'],
                "zhushi_num"=>$data['zhushi_num'],
                "caizhi"=>$data['jinliao'],
                "caizhi_name"=>$data['caizhi'],
                "yanse"=>$data['jinse'],
                "yanse_name"=>$data['yanse'],
                "xiangkou"=>$data['xiangkou'],
                "carat"=>$data['carat'],
                "shoucun"=>$data['shoucun'],
                "xiangqian"=>$data['xiangqian'],
                "facework"=>$data['facework'],
                "jinzhong"=>$data['jinzhong_min']."-".$data['jinzhong_max'],
                "color"=>"",
                "clarity"=>"",                
                "xiangqian" =>$data['xiangqian'],
                "facework"=>$data['facework'],
                "cert"=>$data['cert'],
                "cert_id"=>$data['zhengshu'],
                "goods_type"=>$goods_type,
                "is_xianhuo"=>0,
                "is_dingzhi"=>1,
                "is_cpdz"=>$is_cpdz,
            ...
            $result = $warehouseApi->get_warehousegoods_info($where);
            if(isset($result['error']) && $result['error']==0){
                $data = $result['return_msg'];
            }else{
                return callback(false,$result['error_msg']);
            ...
            if($data['is_on_sale']!=2){
                return callback(false,"货品不是库存状态");
            }else if(($data['order_deail_id']>0 && $goods_type == 4) || ($data['order_goods_id']>0 && $goods_type == 6)){
                return callback(false,"货品已绑定订单");
            }
            
            if(empty($data['goods_image'])) {
                $data['goods_image'] = BASE_SITE_URL."/data/common/default_goods_image_240.gif";
            }
            if($data['product_type']=="钻石" && $data['cat_type']=="裸石"){
                $data['goods_sn'] = "DIA";
            }
            $goods_info = array(
                ...
                'goods_price'=>(int)$data['goods_price'],
                'goods_pay_price'=>(int)$data['goods_price'],
                'goods_hand_price'=>(int)$data['goods_price'],
                'goods_image'=>$data['goods_image'],
                'tuo_type'=>$data['tuo_type']==1?1:2,
                'shape'=>0,
                'shape_name'=>$data['zhushixingzhuang'],
                'zhuchengse'=>$data['caizhi'].$data['jinse'],
                'caizhi'=>0,
                'caizhi_name'=>$data['caizhi'],
                'yanse'=>0,
                'yanse_name'=>$data['jinse'],
                'xiangkou'=>$data['jietuoxiangkou'],
                'zhushi_num'=>$data['zhushilishu'],
                'cert'=>$data['zhengshuleibie']?$data['zhengshuleibie']:'无',                
                'cert_id'=>$data['zhengshuhao'],
                'carat'=>$data['zuanshidaxiao']/1,
                'color'=>$data['yanse'],
                'clarity'=>$data['jingdu'],
                'cut'=>$data['qiegong'],
                'shoucun'=>$data['shoucun']/1,
                "jinzhong"=>$data['jinzhong'],
                "goods_type"=>$goods_type,
                "is_xianhuo"=>1,
                "is_cpdz"=>0,
                "goods_status"=>1,
            ...
            $array = explode("-", $goods_id);
            $style_sn = $array[0];
            $salesApi = data_gateway('isales');
            $result = $salesApi->get_gift_list(array('goods_number'=>$style_sn,'status'=>1,'extends'=>array('goods_image')));
            if(isset($result['error']) && $result['error']==0){
                $data = $result['return_msg'][0];
            }else{
        ...
        $goods_info['discount_code'] = '';   
        $goods_info['goods_type'] = $goods_type;
        $goods_info['goods_storage'] = 999999;
        if($load_member_goods && $member_id>0){
            $member_goods_info = Model("member_goods")->getGoodsInfo(array('goods_id'=>$goods_id,'member_id'=>$member_id,'store_id'=>$store_id,'goods_type'=>$goods_type));
            if(!empty($member_goods_info)){
                unset($member_goods_info['goods_info']);
                $goods_info = array_merge($goods_info,$member_goods_info);
            }
        }
        return callback(true,"查询成功",$goods_info);
    ...
    public function calcCartList($store_cart_list) {
        ...
        foreach ($store_cart_list as $store_id => $store_cart) {
            ...
            foreach ($store_cart as $key => $cart_info) {
                $store_cart[$key]['goods_total'] = ncPriceFormat($cart_info['goods_pay_price'] * $cart_info['goods_num']);
    ...
    public function makeOrderSn($pay_id) {
        ...
        if(!empty(C('sys_scope')) && C('sys_scope')=='tsyd'){
            return '9'. sprintf('%013d', $pay_id) . sprintf('%02d', $num);
        }else{
    ...
    public function getReciverAddr($address_info = array()) {
        if (intval($address_info['dlyp_id'])) {
            ...
            $reciver_info['dlyp'] = 1;
            $reciver_info['provice_id'] = $address_info['provice_id'];
            $reciver_info['city_id'] = $address_info['city_id'];
            $reciver_info['area_id'] = $address_info['area_id'];
            $reciver_info['true_name'] = $address_info['true_name'];
            $reciver_info['chain_id'] = $address_info['chain_id'];
