...
class buyLogic {
     ...
     */
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id,$company_id,$jjg = null,$orderdiscount = 0,$member_level = 0,$ifchain = null) {
        ...
        if ($ifcart) {
            ...
            //购物车列表
            $cart_ids = array_keys($buy_items);
            $condition = array('cart_id'=>array('in',$cart_ids), 'buyer_id'=>$member_id);
            $cart_list  = $model_cart->listCart('db', $condition);
            $_cart_id = array();
            foreach ($cart_list as $vo){
                $_cart_id[] = $vo['goods_id']."@".$vo['goods_type'];
            }
            $cart_id = $_cart_id;
        }
        
        //$result = $this->getGoodsList($cart_id, $member_id, $store_id,$orderdiscounts);
        //切换boss API查询商品列表  by gaopeng 2018-3-30
        $result = $this->apiGetGoodsList($cart_id, $member_id, $store_id,$company_id,$orderdiscounts);
        if(!$result['state']) {
            return $result;
        }
        if ($ifcart) {
            foreach ($result['data']['store_cart_list'] as $k1=>$store_goods_list){
                foreach ($store_goods_list as $k2=>$goods){
                    $goods['cart_id'] = !empty($cart_ids[$k2])?$cart_ids[$k2]:0;
                    if(!empty($cart_list[$k2])){
                        $goods = array_merge($cart_list[$k2],$goods);
                        //var_dump($goods);exit;
                    }
                    $store_goods_list[$k2] = $goods;
                }
                $result['data']['store_cart_list'][$k1] = $store_goods_list;
            }
    ...
    /**
     * 第一步：处理立即购买（重写 $this->getGoodsList 方法）
     *
     * @param array $cart_id 购物车
     * @param int $member_id 会员编号
     * @param int $store_id 店铺编号
     ...
     */
    public function apiGetGoodsList($cart_id, $member_id, $store_id,$company_id,$orderdiscounts = array()) {
        if (empty($cart_id) || !is_array($cart_id)) {
            return callback(false, '所购商品无效');
        }
        $store_cart_list = array();
        $goods_list = array();
        $model_member_goods = Model("member_goods");
        foreach ($cart_id as $vo){
             list($goods_id,$goods_type) = explode("@",$vo);
             
             $res = $this->_logic_buy_1->apiGetGoodsInfo($goods_id,$goods_type,$store_id,$company_id,$member_id,true);
             if(!$res['state']){
             ...
             $goods_list[] = $store_cart_list[$store_id][] = $goods_info;
        }
        //print_r($goods_list);
        return callback(true, '', array('goods_list' => $goods_list, 'store_cart_list' => $store_cart_list,'is_book'=>false));
    }
    ...
    public function buyStep2($post, $member_id, $member_name, $member_email, $orderdiscount = 0, $member_level = 0) {
        ...
        //增加store_id，company_id 
        if(empty($post['store_id']) || empty($post['company_id'])){
            return callback(false,'参数错误：store_id,company_id');
        }
        $this->_member_info['store_id']  = $post['store_id'];
        $this->_member_info['company_id'] = $post['company_id'];
        $this->_member_info['store_id'] = $post['store_id'];
        $this->_member_info['company_id'] = $post['company_id'];
     ...
     */
    public function delMemberGoods($member_id,$store_id,$goods_ids){
        //if (!$member_id || !$store_id || !is_array($goods_ids)) return;
        $condition = array('goods_id'=>array('in',$goods_ids),'member_id'=>$member_id,'store_id'=>$store_id);
        $res = Model('member_goods')->where($condition)->delete();
        //清空1天以前的记录
        $yesterday = date("Y-m-d",strtotime("-1 days"));
        $condition = array('update_time'=>array('lt'=>$yesterday),'member_id'=>$member_id);
        Model('member_goods')->where($condition)->delete();
    }
...
//             $order_platform_store_ids[$k] = null;
    ...
    private function _createOrderStep1() {
        ...
        //收货地址 add by gaopeng 2018-4-8
        if(!empty($post['address_info'])){
            $input_address_info = $post['address_info'];
            $input_city_id = 0;
            if(!empty($input_address_info['city_id'])){
                $input_city_id = $input_address_info['city_id'];
            }
            $input_chain_id = 0;
            if(!empty($input_address_info['chain_id'])){
                $input_chain_id = $input_address_info['chain_id'];
            }
            $input_dlyp_id = 0;
            if ($input_address_info['dlyp_id']) {
                $input_dlyp_id = $input_address_info['dlyp_id'];
        ...
        */        
        ...
        if (false && !$post['chain']['id']) {
            ...
            if (!in_array($input_if_offpay,array('allow_offpay','deny_offpay'))) {
                throw new Exception('订单保存出现异常[货到付款验证错误]，请重试'.$input_if_offpay);
            }
    ...
    private function _createOrderStep2() {
        ...
        $store_id = $this->_member_info['store_id'];
        $company_id = $this->_member_info['company_id'];
        /*if (false && $post['ifcart']) {
            //购物车列表
            $model_cart = Model('cart');
        ...
        if ($post['ifcart']) {
            ...
            $input_buy_items = $_cart_id;
        }
        //来源于直接购买
        $store_cart_list = array();
        $goods_list = array();
        foreach ($input_buy_items as $k=>$v){
            list($goods_id,$goods_type) = explode("@",$v);
            $res = $this->_logic_buy_1->apiGetGoodsInfo($goods_id,$goods_type,$store_id,$company_id,$member_id);
            if($res['state'] == false) {
                throw new Exception($res['error']);
            }else{
                $goods_info = $res['data'];
                if(!empty($cart_list[$k])){
                    $goods_info = array_merge($cart_list[$k],$goods_info);
                }
            }
    ...
    private function _createOrderStep4() {
        ...
        $goodsModel = new goods_itemsModel();//Model('goods_items');
        ...
        if(empty($store_cart_list)){
            throw new Exception('订单保存失败[购物车商品不存在]');
        }
        ...
        $store_id = $this->_member_info['store_id'];
        $source_id = '';
        if(!empty($input_address_info['source_id'])){
            $management_api = data_gateway('imanagement');             
            $res = $management_api->get_customer_sources_list(array('source_codes'=>$input_address_info['source_id']));
            if(!empty($res['return_msg'][0])){
                $source_id = $res['return_msg'][0]['id'];
            }         
        }
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            $order['buyer_id'] = $member_id;//buyer_id=seller_id
            $order['buyer_name'] = $reciver_name;
            //$order['buyer_email'] = $member_email;
            $order['buyer_phone'] = is_numeric($reciver_phone) ? $reciver_phone : 0;            
            $order['seller_id'] = $member_id;
            $order['seller_name'] = $member_name;
            ...
            $order['order_state'] = ORDER_STATE_TO_CONFIRM;
            ...
            $order['customer_source_id'] = $source_id;          
            $order['chain_code'] = $input_chain_id ? $input_chain_id : 0;
            $order['rpt_amount'] = empty($store_rpt_total[$store_id]) ? 0 : $store_rpt_total[$store_id] ;
            $order['remark'] = isset($input_pay_message[$store_id]) ? $input_pay_message[$store_id] : '';
...
//             }
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    $order_goods[$i] = $this->_buildOrderGoods($order_goods[$i],$goods_info);
                    ...
                    if($order_goods[$i]['is_xianhuo']==0){                        
                        $orderEdit['is_xianhuo'] = 0;
                    }
                    ...
                    throw new Exception('抱歉，暂不支持优惠组合套装');
                    //优惠套装
                ...
                if($goods_info['goods_type'] == 2){
                    if(isset($goods_info['cart_id'])){
                        $cart_id = $goods_info['cart_id'];
                    }else{
                        $cart_id = '';
                    }
                    $diainfo_pay = array(
                        'seller_id'=>$goods_info['buyer_id'],
                        'cart_id'=>$cart_id,
                    ...
                    $diamond_pay_log_model ->insert($diainfo_pay);
                }



            }
            if($is_zp_count == count($goods_list)){
                $orderEdit['is_zp'] = 1;
            }
            $orderEdit['order_state'] = ORDER_STATE_TO_CONFIRM;//待确认
            $model_order->editOrder($orderEdit,array('order_id'=>$order_id));
            // 加价购换购商品
            ...
            //下单货品绑定订单 begin
            $order_goodsinfo = $model_order->getOrderGoodsList(array('order_id'=>$order_id));
            $warehouseApi = data_gateway('iwarehouse');
            
            $bind_goods_data =$xiajiainfo= array();
            foreach ($order_goodsinfo as $key => $goodinfo) {   

                if(!empty($goodinfo['goods_itemid'])){
                    $goods_id = $goodinfo['goods_itemid']; 
                    $is_shop = $goodinfo['from_type']==4?1:2;
                    if($is_shop==1){
                        $bind_goods_data[$goods_id] = array(
                            'order_sn'=>$order_sn,
                            'order_detail_id'=>$goodinfo['rec_id'],
                            'is_shop'=>1,          
                        );
                        
                    }else{
                        $bind_goods_data[$goods_id] = array(
                            'kela_order_sn'=>$order_sn,
                            'order_goods_id'=>$goodinfo['rec_id'],
                            'is_shop'=>2,
                        );                        
                    }    
                }
                ...
                $xiajia_dia = array();
                //裸钻下架
                if($goodinfo['style_sn'] == 'DIA' && $goodinfo['goods_type'] == '1'){
                    $xiajia_dia['cert_id'] = $goodinfo['cert_id'];
                    $xiajia_dia['status'] = 2;
                    
                }
                $xiajiainfo[] = $xiajia_dia; 
            ...
            //裸钻下架
            if(!empty($xiajiainfo)){
                $res = $diamondApi->update_diamond_info($xiajiainfo);
                if($res['error']==1){
                    throw new Exception("更新裸钻下架失败！".$res['error_msg']);
                }
            }

            if(!empty($bind_goods_data)){
                $updated_goods_ids = array();
                $res = $warehouseApi->update_warehousegoods_info($bind_goods_data);
                if($res['error'] == 1){
                    //尝试对总部已更绑定状态的货品，进行解绑
                    $unbind_goods_data = array();
                    foreach ($bind_goods_data as $_goods_id => $_goodinfo) {
                        if($_goodinfo['is_shop'] == 2){
                            $unbind_goods_data[$_goods_id] = array(
                                'kela_order_sn'=>'',
                                'order_goods_id'=>0,
                                'is_shop'=>2,
                            );
                    ...
                    if(!empty($unbind_goods_data)){
                        $warehouseApi->update_warehousegoods_info($unbind_goods_data);
                    }
                    throw new Exception("货号绑定订单失败！".$res['error_msg']);
                }
            }
            //下单货品绑定订单 end
            
            //订单日志
            $model_order->addOrderLog(array('order_id' => $order_id,'log_role'=>'seller', 'log_msg' => '创建订单:'.$order_sn, 'log_user' =>$order['seller_name'],'log_orderstate' =>$order['order_state']));
            //客户来源变更,日志
            if(!empty($input_address_info)){
                $source_id = $input_address_info['source_id'];
                $org_source_id = $input_address_info['org_source_id'];
                $source_name = $input_address_info['source_name'];
                $org_source_name = $org_source_id;                
                if($source_id != $org_source_id){
                    if(!empty($org_source_id)){
                        $management_api = data_gateway('imanagement');
                        $res = $management_api->get_customer_sources_list(array('source_codes'=>$org_source_id));
                        if(!empty($res['return_msg'][0])){
                            $org_source_name = $res['return_msg'][0]['source_name'];
                        }
                    }
                    $model_order->addOrderLog(array('order_id' => $order_id,'log_role'=>'seller', 'log_msg' => "客户来源变更,由【{$org_source_name}】变更为【{$source_name}】", 'log_user' =>$order['seller_name'],'log_orderstate' =>$order['order_state']));
                }   
            }
            //存储商家发货提醒数据
     ...
     */
    private function _buildOrderGoods($order_goods,$goods_info){
        //附加字段
        $order_goods['xiangkou'] = null;//1
        $order_goods['zhiquan'] = null;//2
        $order_goods['jinzhong'] = null;//3
        $order_goods['caizhi'] = null;//4
        $order_goods['jinse'] = null;//5
        $order_goods['face_work'] = null;//6
        $order_goods['xiangqian'] = null;//7
        $order_goods['kezi'] = null;//8
        $order_goods['zhushi_num'] = null;//9
        $order_goods['is_cpdz'] = 0;//10
        $order_goods['cpdz_code'] = null;//11
        $order_goods['color'] = null;
        $order_goods['clarity'] = null;
        $order_goods['carat'] = null;
        $order_goods['cut'] = null;
        $order_goods['cert_type'] = null;
        $order_goods['cert_id'] = null;
        $order_goods['is_xianhuo'] = $goods_info['is_xianhuo'];
        $order_goods['is_finance'] = 1;//是否销账
        $order_goods['style_sn'] = $goods_info['style_sn'];
        $order_goods['goods_pay_price'] = $goods_info['goods_pay_price'];
        $order_goods['discount_code'] = $goods_info['discount_code'];
        $order_goods['dia_is_xianhuo'] = 0;
        $order_goods['is_qiban'] = 0;     
        $order_goods['xianhuo_adds'] = $goods_info['xianhuo_adds'];
        $order_goods['goods_itemid'] = '';
        $order_goods['from_type'] = $goods_info['goods_type'];
        $order_goods['zhushi_num'] = intval($goods_info['zhushi_num']);
        if(in_array($goods_info['goods_type'],array(1,4,6))){
            //空托 OR 现货  or 总部现货
            $order_goods['xiangkou'] = $goods_info['xiangkou'];
            $order_goods['zhiquan'] = $goods_info['shoucun'];
            $order_goods['jinzhong'] = $goods_info['jinzhong'];
            $order_goods['caizhi'] = $goods_info['caizhi_name'];
            $order_goods['jinse'] = $goods_info['yanse_name'];
            $order_goods['tuo_type'] = $goods_info['tuo_type'];//金托类型：1成品 2空托
            $order_goods['face_work'] = $goods_info['facework'];//表面工艺
            $order_goods['xiangqian'] = $goods_info['xiangqian'];//镶嵌方式
            $order_goods['kezi'] = $goods_info['kezi'];//刻字内容         
            //
            $order_goods['color'] = $goods_info['color'];
            $order_goods['clarity'] = $goods_info['clarity'];
            $order_goods['carat'] = floatval($goods_info['carat']);
            $order_goods['cut'] = $goods_info['cut'];
            $order_goods['cert_type'] = $goods_info['cert'];
            $order_goods['cert_id'] = $goods_info['cert_id'];
            //
            $order_goods['is_cpdz'] = $goods_info['is_cpdz'];
            $order_goods['cpdz_code'] = $goods_info['cpdz_code'];
            if(in_array($goods_info['goods_type'],array(4,6))){
                $order_goods['goods_itemid'] = $goods_info['goods_id'];
            }
            if($goods_info['goods_type']==6){
                $order_goods['is_xianhuo'] = 0;//总部现货，按期货类型下单
            }
       }else if($goods_info['goods_type']==2){
            //裸钻  
            $order_goods['color'] = $goods_info['color'];
            $order_goods['clarity'] = $goods_info['clarity'];
            $order_goods['carat'] = $goods_info['carat'];
            $order_goods['cut'] = $goods_info['cut'];
            $order_goods['style_sn'] = "DIA";
            $order_goods['tuo_type'] = 1;//1成品
            $order_goods['cert_type'] = $goods_info['cert'];
            $order_goods['cert_id'] = $goods_info['cert_id'];
            $order_goods['is_xianhuo'] = 0;
            $order_goods['dia_is_xianhuo'] = $goods_info['is_xianhuo'];
            $order_goods['xiangqian'] = $goods_info['xiangqian'];//镶嵌方式
      }else if($goods_info['goods_type']==3){
          //起版
          $order_goods['xiangkou'] = $goods_info['xiangkou'];
          $order_goods['zhiquan'] = $goods_info['shoucun'];
          $order_goods['jinzhong'] = $goods_info['jinzhong'];
          ...
          $order_goods['cert_type'] = $goods_info['cert'];
          $order_goods['cert_id'] = $goods_info['cert_id'];
          $order_goods['is_qiban'] = 1;
          $order_goods['is_cpdz'] =  $goods_info['xiangqian'] =='工厂配钻，工厂镶嵌'  ? 1 : 0;
      }else if($goods_info['goods_type']==5){
          //赠品          
          $order_goods['tuo_type'] = 1;//1成品          
          $order_goods['zhiquan'] = $goods_info['shoucun'];
          $order_goods['goods_type'] = 5;
          $order_goods['is_xianhuo'] = 1;
          $order_goods['is_finance'] = $goods_info['is_finance'];//是否销账
      }else{
          output_error("goods_type 不合法！");
      }
      //商品折扣券
      if(!empty($goods_info['discount_code'])){
          $voucher_info = Model('voucher')->getVoucherInfo(array('voucher_code'=>$goods_info['discount_code'],'voucher_type'=>1));
          if(empty($voucher_info)){
              output_error("{$goods_info['discount_code']}折扣码不存在！");
          }else if($voucher_info['voucher_start_date']>time() || $voucher_info['voucher_end_date']<time()){
              output_error("{$goods_info['discount_code']}折扣码不在有效期内！");
          }else if($voucher_info['voucher_store_id']!= $this->_member_info['store_id']){
              output_error("{$goods_info['discount_code']}折扣码不属于当前门店！");
          }else if($voucher_info['voucher_state']!=1){
              output_error("{$goods_info['discount_code']}折扣码已被使用！");
          }
          $this->_order_data['input_goods_voucher_list'][] = array(
              'voucher_id'=>$voucher_info['voucher_id'],
              'voucher_owner_id'=>$this->_member_info['member_id'],
              'voucher_owner_name'=>$this->_member_info['member_name'],
              'voucher_order_id'=>$order_goods['order_id'],
              'voucher_state'=>2,              
          ); 
                 
      }
      //成品定制码
      if(!empty($goods_info['cpdz_code'])){
          $voucher_info = Model('voucher')->getVoucherInfo(array('voucher_code'=>$goods_info['cpdz_code'],'voucher_type'=>2));
          if(empty($voucher_info)){
              output_error("成品定制码不存在！");
          }else if($voucher_info['voucher_state']!=1){
              output_error("成品定制码已被使用！");
          }
          $this->_order_data['input_cpdz_voucher_list'][] = array(
              'voucher_id'=>$voucher_info['voucher_id'],
              'voucher_owner_id'=>$this->_member_info['member_id'],
              'voucher_owner_name'=>$this->_member_info['member_name'],
              'voucher_order_id'=>$order_goods['order_id'],
    ...
    private function _createOrderStep6() {
        ...
        $input_voucher_list = $this->_order_data['input_voucher_list'];
        $input_goods_voucher_list = $this->_order_data['input_goods_voucher_list'];
        $input_cpdz_voucher_list = $this->_order_data['input_cpdz_voucher_list'];
        $input_rpt_info = $this->_order_data['input_rpt_info'];
        ...
        //print_r($input_goods_voucher_list);exit;
        //更新商品折扣券状态
        if (!empty($input_goods_voucher_list) && is_array($input_goods_voucher_list)) {
            $voucher_id_list = array_unique(array_column($input_goods_voucher_list,"voucher_id"));
            if(count($voucher_id_list) != count($input_goods_voucher_list)){
                throw new Exception('商品折扣不能重复使用');
            }
            $result = Logic('queue')->editGoodsVoucherState($input_goods_voucher_list);
            if (!$result['state']) {
                throw new Exception('订单保存失败[商品折扣券处理失败]');
            }
        }
        //更新商品成品定制码状态
        if (!empty($input_cpdz_voucher_list) && is_array($input_cpdz_voucher_list)) {
            $voucher_id_list = array_unique(array_column($input_cpdz_voucher_list,"voucher_id"));
            if(count($voucher_id_list) != count($input_cpdz_voucher_list)){
                throw new Exception('成品定制码不能重复使用');
            }
            $result = Logic('queue')->editGoodsVoucherState($input_cpdz_voucher_list);
            if (!$result['state']) {
                throw new Exception('订单保存失败[商品成品定制码处理失败]');
            }
        }
        //更新使用的订单代金券状态
        if (!empty($input_voucher_list) && is_array($input_voucher_list)) {
            ...
            if (!$result['state']) {
                throw new Exception('订单保存失败[订单代金券处理失败]');
            }
        }        
        //更新使用的平台红包状态
        ...
        //删除用户商品信息
        $this->delMemberGoods($this->_member_info['member_id'],$this->_member_info['store_id'],array_column($goods_list,"goods_id"));
        //保存用户信息
        if(!empty($input_address_info)){
            $address_info = array(
                'member_id'=>$this->_member_info['member_id'],
                'mob_phone'=>$input_address_info['mob_phone'],
                'true_name'=>$input_address_info['true_name'],
                'area_id'=>$input_address_info['area_id'],
                'city_id'=>$input_address_info['city_id'],
                'provice_id'=>$input_address_info['provice_id'],
                'area_info'=>$input_address_info['area_info'],
                'address'=>$input_address_info['address'],
                'is_default'=>1,
                'dlyp_id'=>$input_address_info['chain_id'],
            );            
            $data = array("address_info"=>$address_info);
            $res = QueueClient::push('saveDeliveryAddress', $data);            
        }
        //保存订单自提点信息
        if ($input_dlyp_id) {
            $data = array();
    ...
    private function _createOrderStep7(){
        //print_r($this->_order_data);
        $order_list = $this->_order_data['order_list'];
        
        foreach ($order_list as $order){
            $order_id = $order['order_id'];
            $order_sn = $order['order_sn'];
            $goods_list = Model("order_goods")->where(array('order_id'=>$order_id))->select();
            foreach ($goods_list as $goods){
