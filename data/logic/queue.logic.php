...
class queueLogic {
    ...
    /**
     * 商品折扣券
     * @param unknown $voucher_list
     * @return Ambigous <multitype:unknown, multitype:unknown >
     */  
    public function editGoodsVoucherState($voucher_list) {
        $model_voucher = Model('voucher');
        $send = new sendMemberMsg();
        foreach ($voucher_list as $store_id => $voucher_info) {              
            //$voucher_info['voucher_state'] = 2;
            $update = $model_voucher->editVoucher($voucher_info,array('voucher_id'=>$voucher_info['voucher_id']));
    ...
    public function buildPwdvoucher($t_id){
        ...
        while($t_total > 0){
            ...
            for($t = 0; $t < $step; $t++){
                ...
                while($model_voucher->checkAvailableVoucher($voucher_code)){
                    $voucher_code = $model_voucher->get_voucher_code(0);
                }
                ...
                $tmp['voucher_goods_type'] = $t_info['voucher_t_goods_type'];
                $tmp['voucher_state'] = 1;
                ...
                if(isset($t_info['voucher_t_type'])) $tmp['voucher_type'] = $t_info['voucher_t_type'];
                $insert_arr[] = $tmp;
                $t_total--;
            }
     ...
     */
    public function push_order($param) {
        if((!$param['order_id']) || $param['order_id'] <= 0){
            return callback(false);
        }
        //订单主表，订单明细, 订单支付信息
        $order_id=$param['order_id'];
        $order_info=Model("order")->getOrderInfo(array('order_id'=>$order_id),array("order_common","order_goods","store","order_pay_action"));
        //获取订单信息，发送到后端系统
     ...
     * @param unknown $param
     */
    public function pull_order($param) {
        // 获取需要同步的布产订单列表
        $data = Model("order")->getOrderSyncList(100);
        foreach ($data as $d) {
     ...
     * 添加订单自提表内容
     */
    public function saveDeliveryAddress($param) {
        if(empty($param['address_info'])){
            return callback(true,"参数错误");
        }
        $address_info = $param['address_info'];
        if (empty($address_info['mob_phone'])) {
            return callback(true,"手机号不能为空");
        }
        $mob_phone = $address_info['mob_phone'];
        $data = array();
        $model_address = Model('address');
        $olddo = $model_address->getDefaultAddressInfo(array('mob_phone'=>$mob_phone));
        if(!empty($olddo)){
            $res = $model_address->editAddress($address_info,array('mob_phone'=>$mob_phone));
        }else{
            $res = $model_address->addAddress($address_info);
        }
        //file_put_contents("1.txt",var_export($res,true));
        if($res !==false){
            return callback(true);
        }else{
            return callback(false,"收货地址保存失败");
        }
    }
