...
class search_style{
    ...
    public function __construct(){
        ...
        $this->_app_name = C('fullindexer.appname');
    }
    ...
    public function getIndexerList($condition = array(), $order = array(),$pagesize = 20) {
        try {
            //全文搜索初始化
            $this->_createXS($condition['store_id'], $pagesize);
            //设置搜索内容
    ...
    private function _setQueryXS($condition,$order) {
        ...
        if (isset($condition['style_name'])&&!empty($condition['style_name'])) {
            $this->_xs_search->addQueryString('style_name'.':"'.utf8_encode($condition['style_name']).'"',XS_CMD_QUERY_OP_OR);
            ...
            //是否推荐
           if (isset($condition['is_recommend'])&&!empty($condition['is_recommend'])) {
                $this->_xs_search->addQueryString('is_recommend'.':"'.$condition['is_recommend'].'"');
            }
            //是天生一对
           if (isset($condition['is_tsyd'])&&!empty($condition['is_tsyd'])) {
                $this->_xs_search->addQueryString('is_tsyd'.':"'.$condition['is_tsyd'].'"');
            }
            //查询全部
            if (isset($condition['default'])&&!empty($condition['default'])) {
                $this->_xs_search->addQueryString('default'.':"'.$condition['default'].'"');
            }
            //款式性别
            if (isset($condition['style_sex'])&&!empty($condition['style_sex'])) {
                $this->_xs_search->addQueryString('style_sex'.':"'.$condition['style_sex'].'"');
            }
            ...
            if (isset($condition['zhushizhong'])&&!empty($condition['zhushizhong'])) {
                $this->_xs_search->addQueryString('zhushizhong'.':'.sprintf('%.3f',$condition['zhushizhong']));
            }
            if (isset($condition['style_id'])&&!empty($condition['style_id'])) {
                $this->_xs_search->addQueryString('style_id'.':'.$condition['style_id']);
            }
            if (isset($condition['stone_position'])&&!empty($condition['stone_position'])) {
                $this->_xs_search->addQueryString('stone_position'.':'.$condition['stone_position']);
            }
            ...
            if (isset($condition['stone_cat'])&&!empty($condition['stone_cat'])) {
                if(is_array($condition['stone_cat'])){
                    $this->addMultipleQueryString($condition,"stone_cat");
                }else{
                    $this->_xs_search->addQueryString('stone_cat'.':'.$condition['stone_cat']);
                }
            }
            ...
            if (isset($condition['shoucun_min'])&&!is_null($condition['shoucun_min'])) {
            ...
            if((isset($condition['xiangkou_min'])&&!empty($condition['xiangkou_min']))&&(isset($condition['xiangkou_max'])&&!empty($condition['xiangkou_max']))) {
                $this->_xs_search->addRange('xiangkou',sprintf('%.3f',$condition['xiangkou_min']),sprintf('%.3f',$condition['xiangkou_max']));
            }else if (isset($condition['xiangkou_min'])&&!is_null($condition['xiangkou_min'])&&$condition['xiangkou_min']>0) {
                $this->_xs_search->addRange('xiangkou',sprintf('%.3f',$condition['xiangkou_min']),null);
            }else if(isset($condition['xiangkou_max'])&&($condition['xiangkou_max']=="0"||$condition['xiangkou_max']>0)){
                $this->_xs_search->addRange('xiangkou',null,sprintf('%.3f',$condition['xiangkou_max']));
            }
     ...
     */
    private function _createXS($store_id, $pagesize) {
        ...
        $obj_xs = new XS($this->_app_name.'_'.$store_id);
    ...
    private function _searchXS($page_size){
        ...
        //print_r($count);
        //解决总条数(getLastCount()/count())统计不准确问题,重复查询一次
        ...
        //解决总条数(getLastCount()/count())统计不准确问题,重复查询二次
        $total_page = ceil($count/$page_size);
        $begin = ($total_page-1)*$page_size;
        $this->_xs_search->setCollapse($this->_collapse,1)->setLimit($page_size,$begin);
        $this->_xs_search->setCollapse($this->_collapse,1)->search();
        ...
        foreach ($docs as $index=>$doc) {
            foreach (array('unique_id','goods_id','goods_image','product_type_id','cat_type_id','style_sn','style_name','goods_sn','shoucun',
                    ...
                    'style_id','stone_position','stone_cat','shape','is_recommend','tsyd_style_sn','is_tsyd','default','store_id','style_sex') as $v) {
    ...
    public function indexerSearch($condition = array(), $page_size) {
        ...
        if(isset($condition["order_by"])&&!empty($condition["order_by"])){
            ...
            if (in_array($sort_key,array('1','2','3','4','5'))) {
                $order = array(str_replace(array('1','2','3','4','5'), array('goods_salenum','goods_click','style_id','goods_salenum','goods_price'),$sort_key)
