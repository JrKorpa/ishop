...
<div class="ncsc-flow-layout">
  <div class="ncsc-flow-container">
    ...
    <div class="ncsc-form-default">
      ...
      <dl>
        <dt>退款方式：</dt>
        <dd><?php if($output['refund']['return_type'] == 1){ ?>不退商品<?php }elseif($output['refund']['return_type'] == 2){ ?>退商品<?php }else{?><?php }?></dd>
      </dl>
      <dl>
        <dt>退款类型：</dt>
        <dd><?php if($output['refund']['return_form'] == 1){ ?>现金<?php }elseif($output['refund']['return_form'] == 2){ ?>打卡<?php }elseif($output['refund']['return_form'] == 3){ ?>转单<?php }else{?><?php }?></dd>
      </dl>
    ...
    <dl>
      ...
      <form id="post_form" method="post" action="index.php?act=store_refund&op=edit&refund_id=<?php echo $output['refund']['refund_id']; ?>">
        ...
        <dl>
          ...
          <dd>
            <label class="mr20">
              <input type="radio" class="radio vm" name="seller_state" value="2" checked/>
              同意</label>
            <label>
        ...
        <dl>
          ...
          <dd>
            ...
            <p class="hint">只能提交一次，请认真选择。<br>
              </p><!--同意并经过平台确认后会将金额以预存款的形式返还给买家。<br>
              不同意时买家可以向平台投诉或再次申请。-->
          </dd>
        </dl>
        <div class="bottom">
