...
<div class="tr p10">
    <div style="float:left;font-weight:bold;color: rgba(0, 153, 0, 1)"><?php echo $output['title']; ?></div>
	<input type="hidden" id="export_type<?php echo $output['tag'];?>" name="export_type" data-param='{"url":"<?php echo $output['actionurl'];?>&orderby=<?php echo $output['orderby'];?>&exporttype=excel"}' value="excel"/>
	<a class="ncbtn-mini" id="export_btn<?php echo $output['tag'];?>" href="javascript:void(0);" title="导出Excel">导出Excel</a>
</div>
...
$(document).ready(function(){
	$("#statlist<?php echo $output['tag'];?>").find('.demo').ajaxContent({
		event:'click', //mouseover
		loaderType:"img",
		loadingMsg:"<?php echo SHOP_TEMPLATES_URL;?>/images/transparent.gif",
		target:"#statlist<?php echo $output['tag'];?>"
	});
	...
	//导出图表
    $("#export_btn<?php echo $output['tag'];?>").click(function(){
        var item = $(this).siblings("#export_type<?php echo $output['tag'];?>");
        var type = $(item).val();
        if(type == 'excel'){
        	download_excel(item);
        }
