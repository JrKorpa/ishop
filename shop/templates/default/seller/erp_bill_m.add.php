...
<form method="post" action="index.php?act=erp_bill_m&op=insert" id="erp_bill_m_form">
  <div class="ncsc-form-goods"  <?php if($output['step'] != '1'){?> style="display:none"<?php }?>>
    <input type="hidden" name="bill_m_type" value="ZC"/>
	<dl>
	...
	<dl>
      ...
      <dd>
		...
		<select name="to_house_id" class="chose1">
          <option value=""><?php echo $lang['nc_please_choose'];?></option>		  
        </select>
		</span>
      </dd>
...
var bill_m_type = $("#"+formID+" input[name='bill_m_type']").val();
if(bill_m_type=="ZC"){
   $("#"+formID+" select[name='from_company_id']").val(curr_company_id).attr("disabled",true).change();
   $("#"+formID+" select[name='to_company_id']").val(curr_company_id).attr("disabled",true).change();
   $("#"+formID+" input[name='from_company_id']").val(curr_company_id);
   $("#"+formID+" input[name='to_company_id']").val(curr_company_id);
   $("#to_house_box").show();	
   $('.chosen-container').css('width','33%');	
   get_to_house_list(curr_company_id);   
}else if(bill_m_type=="WX"){
   $("#"+formID+" select[name='from_company_id']").val(curr_company_id).attr("disabled",true).change();
   $("#"+formID+" select[name='to_company_id']").val(58).attr("disabled",true).change();  
   $("#"+formID+" input[name='from_company_id']").val(curr_company_id);
   $("#"+formID+" input[name='to_company_id']").val(58);
   $("#to_house_box").hide();
}
...
function get_to_house_list(to_company_id){
  ...
  $.getJSON('index.php?act=store_warehouse&op=get_house_list_ajax&company_id=' +to_company_id , function(data){  
        if (data) {		    
