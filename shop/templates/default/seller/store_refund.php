...
<table class="ncsc-default-table">
  <thead>
    <tr>
      ...
      <th class="w80">退款类型</th>
      <th class="w80">退款违约金</th>
  ...
  <tbody>
    ...
    <tr class="bd-line" >
        ...
        <td class="w50"><img src="<?php echo empty($val["goods_image"])?$output["default_img_url"]:$val["goods_image"];?>" width="60" height="60" onMouseOver="toolTip('<img src=<?php echo empty($val["goods_image"])?$output["default_img_url"]:$val["goods_image"];?>>')" onMouseOut="toolTip()"/></td>
      ...
      <td><?php if($val['return_form'] == 1){ ?>现金<?php }elseif($val['return_form'] == 2){ ?>打卡<?php }elseif($val['return_form'] == 3){ ?>转单<?php }else{?><?php }?></td>
      <td><?php echo $val['breach_amount']; ?></td>
      <!--<td><?php echo $val['seller_state']==2 ? $output['admin_array'][$val['refund_state']]:'无'; ?></td>-->
      <td class="nscs-table-handle"><?php if ($val['seller_state'] == 1 && $output['return_check']) { ?>
