...
class statistics_achievementControl extends BaseSellerControl {
    private $search_arr;//处理后的参数
    private $api_url;//分类数组


    public function __construct() {
        parent::__construct();

        $this->api_url="http://api.kela.cn/crm/bespoke/view";
    ...
    public function achievementOp(){
        ...
        Tpl::showpage('stat.achievement.achievement');
    }
    ...
    public function achievement_yiyeOp(){
        ...
        Tpl::output('statcount_arr',$statcount_arr);
        Tpl::showpage('stat.achievement.achievement_yiye');
    }
    ...
    public function achievement_zirenOp(){
        ...
        Tpl::output('statcount_arr',$statcount_arr);
        Tpl::showpage('stat.achievement.achievement_ziren');
    }




    public function achievement_listOp(){
    ...
    public function achievement_yiye_listOp(){
        $storeModel = new storeModel();
        $where = array('sc_id'=>2);
        $field = 'store_id,store_name';
        $store_list = $storeModel->getStoreList($where,10,'',$field);
    ...
    public function achievement_ziren_listOp(){
        $storeModel = new storeModel();
        $where = array('sc_id'=>2);
        $field = 'store_id,store_name';
        $store_list = $storeModel->getStoreList($where,10,'',$field);
    ...
    //异业客户
    private function achievement_yiye($store_id){
        $statcount_arr = array();
        $fenlei = 1;
        //成交数
    ...
    //获取任务
    public function getTask($store_id){
        $taskModel = new store_taskModel();
        $year = $this->search_arr['year'];
        $month = $this->search_arr['month'];
        $taskInfo = $taskModel->getStoreTaskInfo(['store_id'=>$store_id,'year'=>$year,'month'=>$month]);
        $task = $taskInfo['task'];
        return $task;
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            ...
            5=>array('menu_key'=>'stock','menu_name'=>'业绩统计','menu_url'=>'index.php?act=statistics_achievement&op=achievement'),
        );
