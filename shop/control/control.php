...
class BaseMemberControl extends Control {
    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'warehouse' => array('name' => '仓储管理', 'child' => array(
                array('name' => '库存查询', 'act'=>'store_goods_items', 'op'=>'index'),
                array('name' => '进货单', 'act'=>'erp_bill_l', 'op'=>'add'),
                array('name' => '调拨单', 'act'=>'erp_bill_m', 'op'=>'add'),
                array('name' => '销售出库单', 'act'=>'erp_bill_s', 'op'=>'add'),
                array('name' => '批发销售门店结算', 'act'=>'store_checkout', 'op'=>'index'),
                array('name' => '销售退货单', 'act'=>'erp_bill_d', 'op'=>'add'),
                array('name' => '退货返厂单', 'act'=>'erp_bill_b', 'op'=>'add'),
                array('name' => '其他出库单', 'act'=>'erp_bill_c', 'op'=>'add'),
                array('name' => '单据查询', 'act'=>'erp_bill', 'op'=>'index'),
                array('name' => '打印货品标签', 'act'=>'store_goods_print', 'op'=>'index'),
                array('name' => '仓库管理', 'act'=>'store_warehouse', 'op'=>'index'),
                array('name' => '无订单退货单', 'act'=>'erp_bill_ha', 'op'=>'index'),
            )),
...
class BaseSellerControl extends Control {
    ...
    public function __construct(){
        ...
        if ($_GET['act'] !== 'seller_login') {
            ...
            if ($sso_config) {
                ...
                $token = cookie($sso_config['token_ck']);
                if (empty($token)) {
                    Util::jump($sso_config['login']);
                    exit;
                }
                ...
                if (!$checked) {
                    ...
                    setNcCookie($sso_config['token_ck'], $checked, 86400);
                } 
            }
     ...
     * 登录
     * @param $member_info
     * @param null $seller_info
     * @param null $seller_id
     * @param null $store_id
     */
    public function execLogin($member_info, $seller_info=null, $seller_id=null, $store_id=null){
        $model_seller = Model('seller');
        if($seller_info==null) {
            $seller_info=$model_seller->getSellerStoreInfo(array("seller.seller_id"=>$seller_id,"seller_store.store_id"=>$store_id));
        }
        
        if($seller_info==null) {
            $sso_config = Util::get_defined_array_var('SSO');
            showMessage('您的账户没有任何商家信息，请确认', $sso_config['logout'], '', 'error');
        }
        
        $model_seller->editSeller(array('last_login_time' => TIMESTAMP, 'store_id' => $seller_info['store_id'], 'seller_group_id' => $seller_info['seller_group_id'], 'is_admin' => $seller_info['is_admin']), array('seller_id' => $seller_info['seller_id']));
        ...
        // --- begin qxy 2018/3/21 --------------------
        $_SESSION['store_company_id'] = $store_info['store_company_id'];
        $_SESSION['store_company_name'] = $store_info['store_company_name'];
        $store_list = model('seller_store')->get_seller_stores($seller_info['seller_id']);
        $_SESSION['store_list'] = array_column($store_list, 'store_id');
        // --- end ------------------------------------
        ...
        if(in_array("erp_bill_dw",$_SESSION['seller_limits'])){
            $_SESSION['seller_limits'][] = "erp_bill_d";
        }
        if(in_array("erp_bill_sw",$_SESSION['seller_limits'])){
            $_SESSION['seller_limits'][] = "erp_bill_s";
        }
        if(in_array("erp_bill_mw",$_SESSION['seller_limits'])){
            $_SESSION['seller_limits'][] = "erp_bill_m";
        } 
        if(in_array("store_order",$_SESSION['seller_limits'])){
            $_SESSION['seller_limits'][] = "store_deliver";
        }
        $_SESSION['seller_limits'] = array_unique($_SESSION['seller_limits']);
    ...
    protected function getSellerMenuList($is_admin, $limits) {
        ...
        if (intval($is_admin) !== 1) {
            ...
            if (intval($is_admin) == 1) { $seller_menu['order']['child'][]=array('name' => '发货', 'act'=>'store_deliver', 'op'=>'index');}
        }
    ...
    private function _getMenuList() {
        $menu_list = array(
            'goods' => array('name' => '商品', 'child' => array(
                ...
                //array('name' => '实物列表', 'act' => 'boss_goods_list', 'op' => 'index'),
                array('name' => '裸钻列表', 'act' => 'diamond_info', 'op' => 'index'),
                array('name' => '赠品列表', 'act' => 'gift_list', 'op' => 'index'),
                array('name' => '裸钻加价率', 'act'=>'diamond_jiajialv', 'op'=>'index'),
            )),
            'order' => array('name' => '订单物流', 'child' => array(
                array('name' => '实物交易订单', 'act'=>'store_order', 'op'=>'index'),
                array('name' => '订单换货', 'act'=>'store_goods_exchange', 'op'=>'index'),
                array('name' => '收款记录', 'act'=>'store_pay_action', 'op'=>'index'),
            ...
            'warehouse' => array('name' => '仓储管理', 'child' => array(
                ...
                array('name' => '调拨单', 'act'=>'erp_bill_m', 'op'=>'add'),
                array('name' => '销售出库单', 'act'=>'erp_bill_s', 'op'=>'add'),                
                array('name' => '批发销售门店结算', 'act'=>'erp_bill_checkout', 'op'=>'index'),
                //array('name' => '销售退货单', 'act'=>'erp_bill_d', 'op'=>'add'),
                array('name' => '退货返厂单', 'act'=>'erp_bill_b', 'op'=>'add'),
                array('name' => '维修入库单', 'act'=>'erp_bill_dw', 'op'=>'add'),
                array('name' => '维修调拨单', 'act'=>'erp_bill_mw', 'op'=>'add'),
                array('name' => '维修出库单', 'act'=>'erp_bill_sw', 'op'=>'add'),              
                array('name' => '盘点单', 'act'=>'erp_bill_w', 'op'=>'add'),
                //array('name' => '其他出库单', 'act'=>'erp_bill_c', 'op'=>'add'),
                array('name' => '仓库管理', 'act'=>'store_warehouse', 'op'=>'index'),
                array('name' => '无订单退货单', 'act'=>'erp_bill_ha', 'op'=>'index'),
            )),
            'promotion' => array('name' => '促销', 'child' => array(
                ...
                array('name' => '折扣码管理', 'act'=>'store_voucher', 'op'=>'templatelist'),
                array('name' => '销售折扣管理', 'act'=>'base_lz_discount_config', 'op'=>'index'),
                //array('name' => '折扣密码授权', 'act'=>'diamond_jiajialv', 'op'=>'index'),
            ...
            */
            ...
            'statistics' => array('name' => '统计结算', 'child' => array(
                ...
                array('name' => '目标任务', 'act'=>'store_task', 'op'=>'storetasklist'),
                array('name' => '商品分析', 'act'=>'statistics_goods', 'op'=>'goodslist'),
                array('name' => '订单赠品列表', 'act'=>'statistics_sale_gift', 'op'=>'index'),
                //array('name' => '运营报告', 'act'=>'statistics_sale', 'op'=>'sale'),
                array('name' => '销售分析', 'act'=>'statistics_sale', 'op'=>'sale'),
                //array('name' => '会员分析', 'act'=>'statistics_member', 'op'=>'member'),
                array('name' => '损益分析', 'act'=>'statistics_loss', 'op'=>'loss'),
                array('name' => '库存分析', 'act'=>'statistics_stock', 'op'=>'stock'),
                array('name' => '业绩统计', 'act'=>'statistics_achievement', 'op'=>'achievement'),
                array('name' => '业绩统计（总）', 'act'=>'statistics_achievement_all', 'op'=>'achievement'),
    ...
    public function check_seller_limit($perm_key) {        
        return checkSellerLimit($perm_key);
    }
}
