...
class store_voucherControl extends BaseSellerControl{
    ...
    public function templatelistOp(){
        ...
        if(trim($_GET['voucher_t_type'])){
            $param['voucher_t_type'] =$_GET['voucher_t_type'];
        }

        if(trim($_GET['voucher_t_goods_type'])){
            $param['voucher_t_goods_type'] =$_GET['voucher_t_goods_type'];
        }
    ...
    private function createExcel($data = array(), $t_info){
        ...
        //header
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'折扣码编码');
        if($t_info['voucher_t_gettype'] == 2){
            $excel_data[0][] = array('styleid'=>'s_title','data'=>'卡密');
        }
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'折扣码名称');
        ...
        $excel_obj->addWorksheet($excel_obj->charset("折扣码模板($voucher_t_title)",CHARSET));
        $excel_obj->generateXML($excel_obj->charset("折扣码模板($voucher_t_title)的折扣码及其卡密",CHARSET).$_GET['curpage']);
    }
    ...
    public function quotaaddOp(){
        if (chksubmit()){
            ...
            //记录店铺费用
            $this->recordStoreCost($current_price * $quota_quantity, '购买折扣码套餐');

            $this->recordSellerLog('购买'.$quota_quantity.'份折扣码套餐，单价'.$current_price.L('nc_yuan'));
    ...
    public function templateaddOp(){
        ...
        if(chksubmit()){
            ...
            $_POST['gettype_sel']="pwd";
            $_POST['txt_template_describe']="";
            $_POST['txt_template_title']=$_POST["voucher_t_type"]==1?"折扣码":"成品定制码";
            $obj_validate->validateparam = array(
                ...
                array("input" => $_POST["voucher_t_goods_type"], "require" => "true", "message" => '请选商品类型'),
            );
            ...
            //验证卡密折扣码发放数量
            $gettype = 'pwd';
            if (intval($_POST['txt_template_total']) > 50){
                $error.= '领取方式为卡密兑换的折扣码，发放总数不能超过50张';
            }
            if ($error){
                showDialog($error,'','error');
            }else {

                //if(bccomp($limit,100,2) == 1 || bccomp($limit,0,2) == -1){
                //    $error_msg = "请输入折扣范围值0-100之间";
                //    showDialog($error_msg,'','error');
                //}
                $voucher_super_users = C('voucher_super_users');
                if (empty($voucher_super_users) || !in_array($_SESSION['member_name'], $voucher_super_users)) {
                    $discountmodel = model("base_lz_discount_config");
                    $where = array(
                        'user_id' => $_SESSION['member_id'],
                        'type' => $_POST["voucher_t_goods_type"],
                        'enabled' => 1
                        );
                    $discountlist = $discountmodel->getBaseLzDiscountConfigInfo($where);
                    if(empty($discountlist)){
                        $error_msg = "您没有此商品的折扣权限，请申请设置并开启！";
                        showDialog($error_msg,'','error');
                    }
                    $limit_zhekou = bcdiv($limit,100,2);
                    if(bccomp($limit_zhekou,$discountlist['zhekou'],2) == -1){
                        $error_msg = "超出最低折扣权限，请找上级生成折扣码！";
                        showDialog($error_msg,'','error');
                    }
                }
                $insert_arr = array();
                ...
                $insert_arr['voucher_t_start_date'] = time();//默认折扣码模板的有效期为当前时间
                ...
                $insert_arr['voucher_t_type'] = $_POST["voucher_t_type"];
                $insert_arr['voucher_t_goods_type'] = $_POST["voucher_t_goods_type"];
                //自定义图片
    ...
    public function templatedelOp(){
        ...
        $param['voucher_t_id'] = $t_id;
        $param['voucher_t_store_id'] = $_SESSION['store_id'];
        $param['voucher_t_giveout'] = array('elt','0');//会员没领取过折扣码才可删除
        $t_info = $model->table('voucher_template')->where($param)->find();
        if (empty($t_info)){
            showMessage(Language::get('wrong_argument'),'index.php?act=store_voucher&op=templatelist','html','error');
        }
    ...
    public function bulidvoucherOp(){
        $t_id = intval($_GET['tid']);
    	if ($t_id <= 0){
    	    showDialog('折扣码生成失败','','error');
    	}
    	//生成卡密折扣码队列
    	QueueClient::push('buildPwdvoucher', $t_id);
    	showDialog('生成折扣码卡密任务已建立，稍后将生成','reload','succ');
    }
    /**
     * 用户中心右边，小导航
     *
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>'折扣码管理', 'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
                );
                break;
            case 'quota_add':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>'折扣码管理', 'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
                4=>array('menu_key'=>'quotaadd','menu_name'=>'新增折扣码', 'menu_url'=>'index.php?act=store_voucher&op=quotaadd')
                );
                break;
            case 'template':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>'折扣码管理', 'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
                2=>array('menu_key'=>'templateadd','menu_name'=>'新增折扣码',   'menu_url'=>'index.php?act=store_voucher&op=templateadd'),
                );
                break;
            case 'templateedit':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>'折扣码管理', 'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
                2=>array('menu_key'=>'templateedit','menu_name'=>'编辑折扣码', 'menu_url'=>''),
                );
                break;
            case 'templateinfo':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>'折扣码管理', 'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
                2=>array('menu_key'=>'templateinfo','menu_name'=>'折扣码详细', 'menu_url'=>''),
                );
                break;
        }
