...
//         Tpl::output('help_list',$help_list);
        ...
        $where = array();
        $where['store_id'] = $_SESSION['store_id'];
        $where['order_state'] = array(array('neq',ORDER_STATE_CANCEL),array('neq',ORDER_STATE_SUCCESS),'and');//计入统计的有效订单
        // 昨日销量
        $where['add_time'] = array('between',array(strtotime(date('Y-m-d',(time()-3600*24))),strtotime(date('Y-m-d',time()))-1));
        $daily_sales = Model('order')->getoneByorder($where, $field);
        Tpl::output('daily_sales', $daily_sales);
        // 月销量
        $where['add_time'] = array('gt',strtotime(date('Y-m',time())));
        $monthly_sales = Model('order')->getoneByorder($where, $field);
        Tpl::output('monthly_sales', $monthly_sales);
        unset($field,$where);*/
     ...
     * 更换店铺
     */
    public function change_storeOp() {
        if (chksubmit()) {
            $store_id=$_POST["store_id"];
            if(empty($store_id)) showMessage('请选择店铺', '', '', 'error');
            $seller_id=$_SESSION["seller_id"];
            $model_member = Model('member');
            $member_info = $model_member->getMemberInfo(array('member_id' => $_SESSION['member_id'], 'member_name' =>$_SESSION['member_name']));
            parent::execLogin($member_info,null,$seller_id,$store_id);
            $this->recordSellerLog('切换店铺成功');
            redirect('index.php?act=seller_center');
        }
        $store_model=Model("store");
        $store_list=$store_model->getStoreList(array("store_id"=>array("in",$_SESSION['store_list'])));
        Tpl::output('store_list', $store_list);
        Tpl::showpage('seller_center.change_store','null_layout');
    }
    /**
     * 修改密码
     */
    public function change_pwdOp(){
        $model_member = new memberModel();//Model('member');
        $member_info = $model_member->getMemberInfo(array('member_id'=>$_SESSION['member_id']));
        if(empty($member_info)){
            redirect('index.php?act=seller_center');
        }
        if (chksubmit()) {  
            if(xmd5($_POST['old_password']) != $member_info['member_passwd']){
                $result = callback(false,"原始密码错误");
            ...
            if ($error != ''){
                $result = callback(false,$error);
                exit(json_encode($result));
            }
            $update = $model_member->editMember(array('member_id'=>$_SESSION['member_id']),array('member_passwd'=>xmd5($_POST['password'])));
            $message = $update ? '密码修改成功' : '密码修改失败';
            $this->recordSellerLog($message);
            $result = callback($update?true:false,$message);
            exit(json_encode($result));
        }
        Tpl::output('member_info', $member_info);
        Tpl::showpage('seller_center.change_pwd','null_layout');
    }
}
