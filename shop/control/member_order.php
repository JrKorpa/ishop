...
class member_orderControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            // 显示追加评价
            $order_info['if_evaluation_again'] = $model_order->getOrderOperateState('evaluation_again',$order_info, $store_id);

            //显示删除订单(放入回收站)
            $order_info['if_delete'] = $model_order->getOrderOperateState('delete',$order_info, $store_id);

            //显示永久删除
            $order_info['if_drop'] = $model_order->getOrderOperateState('drop',$order_info, $store_id);

            //显示还原订单
            $order_info['if_restore'] = $model_order->getOrderOperateState('restore',$order_info, $store_id);
    ...
    public function show_orderOp() {
        ...
        $store_id = $_SESSION['store_id'];
        $result = $logic_order->getMemberOrderInfo($_GET['order_id'],$_SESSION['member_id'], $store_id);
        if (!$result['state']) {
    ...
    private function _order_cancel($order_info, $post) {
        ...
        if (!chksubmit()) {
            ...
            $model_order = Model('order');
            $logic_order = Logic('order');
            $if_allow = $model_order->getOrderOperateState('buyer_cancel',$order_info, $store_id);
            if (!$if_allow) {
    ...
    private function _order_receive($order_info, $post) {
        ...
        if (!chksubmit()) {
            ...
            $model_order = Model('order');
            $logic_order = Logic('order');
            $if_allow = $model_order->getOrderOperateState('receive',$order_info, $store_id);
            if (!$if_allow) {
                return callback(false,'无权操作');
            }
    ...
    private function _order_recycle($order_info, $get) {
        ...
        $if_allow = $model_order->getOrderOperateState($state_type,$order_info, $store_id);
        if (!$if_allow) {
            return callback(false,'无权操作');
        }
