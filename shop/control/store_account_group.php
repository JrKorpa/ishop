...
class store_account_groupControl extends BaseSellerControl {
    ...
    private function profile_menu($menu_key='') {
        ...
        $seller_menu = $this->getSellerMenuList(true, []); //$_SESSION['seller_menu'];
        $seller_menu = $seller_menu['seller_menu'];
        $ext_limits = C('extend_limits');
        foreach ($ext_limits as $md => $list) {
            foreach ($list as $li) {
                $seller_menu[$md]['child'][] = $li;
            }
        }
        Tpl::output('seller_menu', $seller_menu);
