...
class store_deliverControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        $order_state = str_replace(array('deliverno','delivering','delivered'),
                array(ORDER_STATE_TOSEND,ORDER_STATE_SEND,ORDER_STATE_SEND),$_GET['state']);
        $condition = array();
    ...
    public function sendOp(){
        ...
        if(!empty($order_info['extend_order_goods'])){
            ...
            foreach ($extend_order_goods as $k => $v){
                if($v['is_return'] ==1){
                    unset($extend_order_goods[$k]);
                    continue;
                }
            }

            $order_info['extend_order_goods'] = $model_order->buildCpdzGoodsList($extend_order_goods);
        }
        
        $if_allow_send = intval($order_info['lock_state']) || !in_array($order_info['order_state'],array(ORDER_STATE_PAY,ORDER_STATE_TOSEND,ORDER_STATE_SEND));
        if ($if_allow_send) {
            showMessage(Language::get('wrong_argument'),'','html','error');
        }
        ...
        if (chksubmit()){
            ...
            if (!$result['state']&&!empty($result['msg'])) {
                showDialog($result['msg'],'','error');
            } else {
                showDialog('操作成功',$_POST['ref_url'],'succ');
            }
        }
